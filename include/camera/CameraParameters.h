...
typedef enum
{
    E_TRAVELING_UNSUPPORT,
    E_TRAVELING_SUCCESS,
    E_TRAVELING_SET_WINDOW_ERROR,
    E_TRAVELING_ALREADY_OPENED,
    E_TRAVELING_INPUT_PARAMETER_ERROR,
    E_TRAVELING_UNKNOWN_ERROR, //For unknown or strange error
} EN_TRAVELING_RETURN;

typedef enum
{
    E_TRAVELING_INVALID_WINDOW = 0,
    ///< Traveling main window only
    E_TRAVELING_MAIN_WINDOW,
    ///< Traveling sub window only
    E_TRAVELING_SUB_WINDOW,
    ///< Traveling all windows on screen
    E_TRAVELING_ALL_WINDOW,
    E_TRAVELING_WINDOW_MAX
} EN_TRAVELING_WINDOW_TYPE;

typedef enum
{
    E_TRAVELING_CAP_STAGE_AUTO,            ///Auto decide the capture stage for better quality
    E_TRAVELING_CAP_STAGE_SIGNAL_INPUT,     ///Force to capture from the signal input stage
    E_TRAVELING_CAP_STAGE_SIGNAL_TVOUTPUT,  ///Force to capture from the signal output stage with TV
    E_TRAVELING_CAP_STAGE_MAX,            ///Maximum capture stage
} EN_TRAVELING_CAPTURE_STAGE;

// VE & XCDip Capture definition
typedef enum {
    E_TRAVELING_MODE_INVALID = 0x00,
    E_TRAVELING_ALL_VIDEO_WITH_OSD,    //Capture all screen video + osd
    E_TRAVELING_ALL_VIDEO,            // Capture all screen video only
    E_TRAVELING_2ND_VIDEO_AUTO,        //Capture Sub source, capture interlace sources as top/bottom field
    E_TRAVELING_2ND_VIDEO_PROGRESSIVE_AUTO,    //Capture sub source, for interlace sources capture to progressive frame(may use PQ or XC function to de-interlace and scaling up/down)
    E_TRAVELING_1ST_VIDEO_AUTO, 	       //Capture main source, for interlace sources capture as top/bottom field
    E_TRAVELING_1ST_VIDEO_PROGRESSIVE_AUTO,    //Capture main source, for interlace sources capture to progressive frame(may use PQ or XC function to de-interlace and scaling up/down)
    E_TRAVELING_MODE_MAX,
} EN_TRAVELING_MODE;

typedef enum {
    E_TRAVELING_RES_720_576, //Std PAL
    E_TRAVELING_RES_320_240, //CIF NTSC
    E_TRAVELING_RES_720_480, //Std NTSC
    E_TRAVELING_RES_640_480, //4CIF NTSC / VGA
    E_TRAVELING_RES_RESERVE0, //reserved
    E_TRAVELING_RES_480_320, //HVGA
    E_TRAVELING_RES_RESERVE1, //reserved
    E_TRAVELING_RES_176_144, //QCIF PAL
    E_TRAVELING_RES_RESERVE2, //reserved
...
typedef enum {
    E_TRAVELING_MEM_FORMAT_YUV422_YUYV,
    E_TRAVELING_MEM_FORMAT_ARGB8888,
    E_TRAVELING_MEM_FORMAT_ABGR8888,
    E_TRAVELING_MEM_FORMAT_YC420,
    E_TRAVELING_MEM_FORMAT_MAX,
} EN_TRAVELING_MEM_FORMAT;

typedef enum {
    E_TRAVELING_SPEED_SLOW,
    E_TRAVELING_SPEED_MIDDLE,
    E_TRAVELING_SPEED_FAST,
    E_TRAVELING_SPEED_MAX,
} EN_TRAVELING_CAPTURE_SPEED;

typedef enum
{
    EN_TRAVELING_CALLBACK_NO_SIGNAL,            ///Traveling source has no signal now
    EN_TRAVELING_CALLBACK_VIDEO_PROGRESSIVE,     ///Traveling source has no signal now
    EN_TRAVELING_CALLBACK_VIDEO_INTERLACE,       ///Traveling source has no signal now
    EN_TRAVELING_CALLBACK_UN_SUPPORT,           ///Traveling source has no signal now
    E_TRAVELING_CALLBACK_MAX,
} EN_TRAVELING_CALLBACK_MSG_TYPE;

typedef enum
{
    E_TRAVELING_EVENT_CALLBACK_NO_SIGNAL,            /// Traveling source has no signal now
    E_TRAVELING_EVENT_CALLBACK_UN_SUPPORT,           /// Traveling unsupport
    E_TRAVELING_EVENT_CALLBACK_STREAM_INFO,          /// Traveling some stream info change
    E_TRAVELING_EVENT_CALLBACK_TVOS_FAST_QUIT,
    E_TRAVELING_EVENT_CALLBACK_MAX,
} EN_TRAVELING_EVENT_CALLBACK_MSG_TYPE;

// DWIN Capture definition
typedef enum {
    E_DWIN_CAPTURE_INVALID = 0x00,
    E_DWIN_CAPTURE_OP_ALL,
    E_DWIN_CAPTURE_OP_VIDEO,
    E_DWIN_CAPTURE_IP,
    E_DWIN_CAPTURE_SCREEN_MAX,
} EN_DWIN_CAPTURE_MODE;
...
public:
    ...
    // MStar Android Patch Begin
    void getSupportedThumbnailPictureSizes(Vector<Size> &sizes) const;
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    // Mstar special parameter keys for vecapture hardware fake camera (only for vecapture)
    static const char KEY_TRAVELING_MODE[];
    static const char KEY_TRAVELING_RES[];
    static const char KEY_TRAVELING_MEM_FORMAT[];
    static const char KEY_TRAVELING_SPEED[];
    static const char KEY_TRAVELING_MAGIC_KEY[];
    static const char KEY_TRAVELING_FRAMERATE[];
    static const char KEY_MAIN_INPUT_SOURCE[];
    static const char KEY_SUB_INPUT_SOURCE[];

    // Mstar special parameter keys for vecapture hardware fake camera (only for dwincapture)
    static const char KEY_DWIN_CAPTURE_MODE[];
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    static const char PIXEL_FORMAT_YUV420TILE[];
    // MStar Android Patch End
    static const char PIXEL_FORMAT_RGB565[];
    static const char PIXEL_FORMAT_RGBA8888[];
    // MStar Android Patch Begin
    static const char PIXEL_FORMAT_ABGR8888[];
    // MStar Android Patch End
    ...
    static const char PIXEL_FORMAT_AVC[];
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    // Values for mstar special parameter key:  KEY_TRAVELING_MODE.
    static const char TRAVELING_SCREEN_WITH_OSD[];
    static const char TRAVELING_SCREEN_WITHOUT_OSD[];
    static const char TRAVELING_2ND_SCREEN_WITHOUT_OSD[];

    // Values for mstar special parameter key:  KEY_TRAVELING_RES.
    static const char TRAVELING_RES_720_576[];
    static const char TRAVELING_RES_320_240[];
    static const char TRAVELING_RES_720_480[];
    static const char TRAVELING_RES_640_480[];

    // Values for mstar special parameter key  KEY_TRAVELING_MEM_FORMAT.
    static const char TRAVELING_MEM_FORMAT_YUV422_YUYV[];
    static const char TRAVELING_MEM_FORMATARGB8888[];
    static const char TRAVELING_MEM_FORMAT_ABGR8888[];

    // Values for mstar special parameter key  KEY_TRAVELING_SPEED.
    static const char TRAVELING_SPEED_SLOW[];
    static const char TRAVELING_SPEED_MIDDLE[];
    static const char TRAVELING_SPEED_FAST[];
    // MStar Android Patch End
