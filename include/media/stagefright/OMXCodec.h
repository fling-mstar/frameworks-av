...
struct OMXCodec : public MediaSource,
    ...
    enum CreationFlags {
        ...
        // MStar Android Patch Begin
        // media source is camera source: include ve camera and WebCam
        kMediaSourceCamera = 512,
        // Streaming Playback
        kIgnoreAvcFrameGap = 1024,
        // Live Streaming Playback
        kLiveStreamingPlayback = 2048,
        // MStar Specific Flag
        kMstarSpecificFlag = 4096,
        // MStar Specific Flag - player uses getFormat to get vdec info fast
        kMstarInitDecFast = 8192,
        // MStar Specific Flag - force decode mode for vdec
        kForceDecode = 16384,
        // MStar Android Patch End
    };
    ...
    // MStar Android Patch Begin
    virtual void setTrickMode(TrickMode mode);
    // MStar Android Patch End
...
private:
    ...
    // MStar Android Patch Begin
    status_t setWMAFormat(OMX_AUDIO_PARAM_WMATYPE *pstWmaInfo);
    status_t setRAFormat(OMX_AUDIO_PARAM_RATYPE *pstRaInfo);
    status_t setADPCMFormat(OMX_AUDIO_PARAM_ADPCMTYPE *pstAdpcmInfo);
    // MStar Android Patch End
    ...
    status_t updateBitrate(int32_t bitRate);
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    // Wasabi fields for secure path
    bool packCryptoInfo(sp<ABuffer> buffer, sp<MetaData> crypto, size_t pad_size);
    bool DecryptBuffer(sp<ABuffer> buffer, sp<MetaData> crypto);
    sp<ABuffer>                    mKey;
    AP4_CencSingleSampleDecrypter* mDecrypter;
    AP4_BlockCipherFactory*        mBlockCipherFactory;
    // MStar Android Patch End
