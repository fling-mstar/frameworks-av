...
status_t MediaRecorder::setOutputFormat(int of)
{
    ...
    // add "OUTPUT_FORMAT_VVOIP" and "OUTPUT_FORMAT_LIVECAP_VIDEO"
    if (mIsVideoSourceSet && of >= OUTPUT_FORMAT_AUDIO_ONLY_START && of != OUTPUT_FORMAT_RTP_AVP && of != OUTPUT_FORMAT_MPEG2TS && of != OUTPUT_FORMAT_VVOIP && of != OUTPUT_FORMAT_LIVECAP_VIDEO) { //first non-video output format
...
status_t MediaRecorder::setParameters(const String8& params) {
    ...
    status_t ret = mMediaRecorder->setParameters(params, (mCurrentState & MEDIA_RECORDER_RECORDING ? MEDIA_RECORDER_RECORDING : 0));
    if (OK != ret) {
        ALOGE("setParameters(%s) failed: %d, state: %x", params.string(), ret, mCurrentState);
