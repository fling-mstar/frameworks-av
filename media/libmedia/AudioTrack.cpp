...
#ifdef FIXED_KEY_VOICE 
    // MStar Android Patch Begin
    // RR_690 Android IR key voice maintained at a fixed value
    if(mStreamType==AUDIO_STREAM_SYSTEM && left != 0 && right != 0 )
    {
        const sp<IAudioFlinger>& aFlinger = AudioSystem::get_audio_flinger();
        float master_volume = aFlinger->masterVolume();
        float vol_adjust_factor;

        //note: the curve below should be the inverse value with respect to
        // android system's master volume (which applied to audio DSP in
        // Edison), but since volume curve is not linear by its natuare, we construct a
        //counter-master-volume-curve here to assist this feature (fixed system sound
        // volume). Customs can adjust this curve according to their specific requirements.
        float v_curve[100] = {
             0.2000, 0.2020, 0.2041, 0.2062, 0.2083, 0.2105, 0.2128, 0.2151, 0.2274, 0.2398,
             0.2444, 0.2472, 0.2500, 0.2529, 0.2558, 0.2688, 0.2719, 0.2751, 0.2883, 0.2916,
             0.3000, 0.3038, 0.3077, 0.3117, 0.3158, 0.3200, 0.3243, 0.3288, 0.3333, 0.3380,
             0.3571, 0.3623, 0.3676, 0.3731, 0.3788, 0.3846, 0.3906, 0.3968, 0.4032, 0.4098,
             0.4500, 0.4576, 0.4655, 0.4737, 0.4821, 0.4909, 0.5000, 0.5094, 0.5192, 0.5294,
             0.5800, 0.5918, 0.6042, 0.6170, 0.6304, 0.6444, 0.6591, 0.6744, 0.6905, 0.7073,
             0.7500, 0.7692, 0.7895, 0.8108, 0.8333, 0.8571, 0.8824, 0.9091, 0.9375, 0.9677,
             1.0000, 1.0345, 1.0714, 1.1111, 1.1538, 1.2000, 1.2500, 1.3043, 1.3636, 1.4286,
             1.5000, 1.5789, 1.6667, 1.7647, 1.8750, 2.0000, 2.1429, 2.3077, 2.5000, 2.7273,
             3.0000, 3.3333, 3.7500, 4.2857, 5.0000, 6.0000, 7.5000,10.0000,14.0000,15.0000
             };

        if (aFlinger == 0) {
            ALOGE("audio track set volume could not get audioflinger.. give up..");
        }
        else {
            //ALOGV("AudioTrack::setVolume left[%f].right[%f]",left,right);
            vol_adjust_factor = v_curve[100 - (int)(master_volume*100)];//*2;
            //if (vol_adjust_factor > 1.0) vol_adjust_factor = 1.0f; //clip max factor @1.0f
            left = left*vol_adjust_factor;
            right = right*vol_adjust_factor;
            ALOGV("track volume adjusted : master[%f].factor[%f].left[%f].right[%f]",master_volume,vol_adjust_factor,left,right);
        }
    }
...
/* mstar add for get tiny ALSA latency start */
uint32_t AudioTrack::getTinyALSALatency()
{
    if (mSharedBuffer != 0) return 0;
    if (mIsTimed) return 0;
    ...
    uint32_t latency = 0;
    latency = audioFlinger->latency(getOutput());

    return latency;
}
/* mstar add for get tiny ALSA latency end */
#endif
// MStar Android Patch End
