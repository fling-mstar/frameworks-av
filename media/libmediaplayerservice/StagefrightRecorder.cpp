...
//#include <media/stagefright/VVOIPWriter.h>
...
status_t StagefrightRecorder::setOutputFormat(output_format of) {
    ...
    // MStar Android Patch Begin
    if (mOutputFormat == OUTPUT_FORMAT_VVOIP) {
        mIsOutputVoip = true;
    }

    if (mOutputFormat == OUTPUT_FORMAT_LIVECAP_VIDEO) {
        mIsOutputLivecap_Video = true;
    }
    // MStar Android Patch End
...
// MStar Android Patch Begin
static sp<IBinder> livecap_binder = NULL;
static ssize_t livecapServiceDatacb(void *cookie, const void *buffer, size_t size) {
    Parcel data, reply;

    if (livecap_binder == NULL) {
        sp<IServiceManager> sm = defaultServiceManager();
        livecap_binder = sm->getService(String16("livecap.rtp"));
        if (livecap_binder == 0) {
            ALOGE("livecap.rtp not published... ");
            return 0;
        }
    }
    ...
    livecap_binder->transact(LIVECAP_VIDEO_OUTPUT_BUF, data, &reply);
...
status_t StagefrightRecorder::setParamVideoEncodingBitRate(int32_t bitRate) {
    ...
    if(mVideoEnc != NULL) {
        sp<MetaData> meta = mVideoEnc->getFormat();
...
status_t StagefrightRecorder::setParameter(
        const String8 &key, const String8 &value, int state) {
    ALOGV("setParameter: key (%s) => value (%s)", key.string(), value.string());
    if (state) {
        if (key == "video-param-encoding-bitrate") {
            ALOGV("change video bitrate at recording state");
        } else {
            ALOGE("setParameters %s is not valid at %x", key.string(), state);
...
// MStar Android Patch Begin
// update encoding settings runtime
status_t StagefrightRecorder::setParameters(const String8 &params, int state) {
    ...
    for (;;) {
        ...
        if (setParameter(key, value, state) != OK) {
            return BAD_VALUE;
        }
        if (semicolon_pos == NULL) {
...
status_t StagefrightRecorder::start() {
    // MStar Android Patch Begin
    // VOIP and LiveCap are exclusive to check CHECK_GE
    if ((mIsOutputVoip == false) && (mIsOutputLivecap_Video == false)) {
    ...
    switch (mOutputFormat) {
        ...
        // MStar Android Patch Begin
        case OUTPUT_FORMAT_LIVECAP_VIDEO:
            if (mIsOutputLivecap_Video == true) {
                status = startVECaptureRecording();
            }
            break;
        // MStar Android Patch Begin
...
status_t StagefrightRecorder::startMPEG2TSRecording() {
    ...
    // MStar Android Patch Begin
    sp<MediaWriter> writer = NULL;
    if (mCameraId >= 3 && mOutputFd == -1)
        writer = new MPEG2TSWriter((void *)mCameraId , livecapServiceDatacb);
    else
...
status_t StagefrightRecorder::setupVideoEncoder(
    ...
    if (OMX_COLOR_FormatEncodedAVC == colorFormat) {    // add condition to run the sequence
            const char *subMime = NULL;
            CHECK_EQ(mVideoEncoder, VIDEO_ENCODER_H264);
            enc_meta->findCString(kKeyMIMEType, &subMime);
            // set to kKeySubMIMEType immediately, otherwise, conent in subMime will be replaced
            enc_meta->setCString(kKeySubMIMEType, subMime);
            enc_meta->findCString(kKeySubMIMEType, &subMime);
            ALOGI("subMime for ByPass Encoder: %s\n", subMime);
            enc_meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_BYPASS);
    }
    // MStar Android Patch End
    OMXClient client;
    CHECK_EQ(client.connect(), (status_t)OK);
    ...
    // MStar Android Patch Begin
    // update encoding settings runtime
    sp<MetaData> metadata = encoder->getFormat();
    metadata->setInt32(kKeyBitRate, videoBitRate);
    mVideoEnc = encoder;
    // MStar Android Patch End
...
// MStar Android Patch Begin
// VECapture and VVOIP
status_t StagefrightRecorder::startVECaptureRecording() {
    int32_t totalBitRate = 0;
    status_t err = OK;
    sp<MediaWriter> writer;

    ALOGV("startVECaptureRecording ->\n");

    // similar with setupMPEG4Recording
    {
        writer = new VECaptureWriter(mFrameRate,
                                    (void *)mCameraId , livecapServiceDatacb);
