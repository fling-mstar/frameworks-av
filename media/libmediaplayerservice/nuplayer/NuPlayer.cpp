...
#ifdef BUILD_WITH_MSS_PLAYREADY
#include "MSSSource.h"
#endif
#ifdef BUILD_WITH_MARLIN
#include <WasabiSource.h>
...
// MStar Android Patch Begin
#ifdef BUILD_WITH_MSS_PLAYREADY
static bool IsMSSSource(const char *url) {
     if (!strncasecmp(url, "abc://", 6)) {
         ALOGE("Hard code MSS url.");
         return true;
     }

     if (!strncasecmp("http://", url, 7)) {
        size_t len = strlen(url);
        if (len >= 13 && !strcasecmp(".ism/Manifest", &url[len - 13])) {
            ALOGV("Got MSS url.");
            return true;
        }
    }
...
#ifdef BUILD_WITH_MSS_PLAYREADY
    if (IsMSSSource(url)){
       source = new MSSSource(notify, url, headers, mUIDValid, mUID);
    } else
#endif
// MStar Android Patch End
...
// MStar Android Patch Begin
#ifdef BUILD_WITH_MSS_PLAYREADY
                    if (mSourceFlags & Source::FLAG_NEED_POSITION_UPDATE) {
                        mSource->updatePositionUs(positionUs);
                    }
#endif
// MStar Android Patch End
                }
