...
status_t MediaRecorderClient::setParameters(const String8& params, int state) {
    ALOGV("setParameters(%s), state: %x", params.string(), state);
    Mutex::Autolock lock(mLock);
    if (mRecorder == NULL) {
        ALOGE("recorder is not initialized");
        return NO_INIT;
    }
    return mRecorder->setParameters(params, state);
}
