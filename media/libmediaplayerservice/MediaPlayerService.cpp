...
{
    // MStar Android Patch Begin
    ALOGD("setDataSource fd=%d, offset=%lld, length=%lld", fd, offset, length);
    // MStar Android Patch End
    struct stat sb;
    int ret = fstat(fd, &sb);
    if (ret != 0) {
...
// MStar Android Patch Begin
    ...
    if(len > 0)
    {
        if(strcasestr(buf,"m3u8")!= NULL)
        {
            player_type playerType = MediaPlayerFactory::getPlayerType(this, buf);
            ALOGD("playerType =%d",playerType);
            sp<MediaPlayerBase> p = setDataSource_pre(playerType);
            if (p == NULL) {
                return NO_INIT;
            }

            setDataSource_post(p, p->setDataSource(buf,NULL));
...
{
    // MStar Android Patch Begin
    Mutex::Autolock lock(mAudioOutputLock);
    // MStar Android Patch End
