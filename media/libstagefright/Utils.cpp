...
status_t convertMetaDataToMessage(
    ...
    if (!strncasecmp("video/", mime, 6)) {
        ...
        int32_t timeStampMode;
        if (meta->findInt32(kKeyMsTimeStampMode, &timeStampMode)) {
            msg->setInt32("time-stamp-mode", timeStampMode);
        }
        ...
        if (!strcasecmp(mime, "audio/x-ms-wma")) {
            int32_t profile;
            if (meta->findInt32(kKeyMsWMAProfile, &profile)) {
            ...
            int32_t blockAlign;
            if (meta->findInt32(kKeyMsWMABlockAlign, &blockAlign)) {
            ...
            int32_t channelMask;
            if (meta->findInt32(kKeyMsWMAChannelMask, &channelMask)) {
                msg->setInt32("channel-mask", channelMask);
            }

            int32_t encodeOpt;
            if (meta->findInt32(kKeyMsWMAEncodeOptions, &encodeOpt)) {
                msg->setInt32("encode-option", encodeOpt);
            }

            int32_t advanceEncodeOpt;
            if (meta->findInt32(kKeyMsWMAAdvancedEncodeOptions, &advanceEncodeOpt)) {
                msg->setInt32("advance-encode-option", advanceEncodeOpt);
            }
        }
        // MStar Android Patch End
    }
...
void convertMessageToMetaData(const sp<AMessage> &msg, sp<MetaData> &meta) {
    ...
    if (mime.startsWith("video/")) {
        ...
        // MStar Android Patch Begin
        int32_t timeStampMode;
        if (msg->findInt32("time-stamp-mode", &timeStampMode)) {
            meta->setInt32(kKeyMsTimeStampMode, timeStampMode);
        }
        // MStar Android Patch End
        ...
        if (!strcasecmp(mime.c_str(), "audio/x-ms-wma")) {
            ...
            if (msg->findInt32("profile", &profile)) {
                meta->setInt32(kKeyMsWMAProfile, profile);
            }
            ...
            if (msg->findInt32("block-align", &blockAlign)) {
                meta->setInt32(kKeyMsWMABlockAlign, blockAlign);
            }

            int32_t bitsPerSample;
            if (msg->findInt32("bits-sample", &bitsPerSample)) {
                meta->setInt32(kKeyMsBitsPerSample, bitsPerSample);
            }

            int32_t channelMask;
            if (msg->findInt32("channel-mask", &channelMask)) {
                meta->setInt32(kKeyMsWMAChannelMask, channelMask);
            }

            int32_t encodeOpt;
            if (msg->findInt32("encode-option", &encodeOpt)) {
                meta->setInt32(kKeyMsWMAEncodeOptions, encodeOpt);
            }

            int32_t advanceEncodeOpt;
            if (msg->findInt32("advance-encode-option", &advanceEncodeOpt)) {
                meta->setInt32(kKeyMsWMAAdvancedEncodeOptions, advanceEncodeOpt);
            }
        }
        // MStar Android Patch End
    }
    ...
    if (msg->findBuffer("csd-0", &csd0)) {
        if (mime.startsWith("video/")) { // do we need to be stricter than this?
            ...
            // MStar Android Patch Begin
            if (!strcasecmp(mime.c_str(), "video/vc1") ||
                !strcasecmp(mime.c_str(), "video/wvc1")) {
                meta->setData(kKeyMsDecConfigData, 0, csd0->data(), csd0->size());
            }
            // MStar Android Patch End
        } else if (mime.startsWith("audio/")) {
            int csd0size = csd0->size();
