...
#include "include/ByPassEncoder.h"
...
#include <WsbSkbAp4BlockCipher.h>
#include <Ap4CommonEncryption.h>
#include <media/openmax/OMX_CoreExt.h>
#include <ms/MS_OMX_Def.h>
#endif

#define ENABLE_AUDIO_HW_DECODE    1
// MStar Android Patch End
...
struct OMXCodecObserver : public BnOMXObserver {
    ...
    virtual void onMessage(const omx_message &msg) {
        ...
        if (codec.get() != NULL) {
            ...
            // MStar Android Patch Begin
            // prevent signalBufferReturned cannot lock mutex for a long time
            usleep(0);
            // MStar Android Patch End
...
uint32_t OMXCodec::getComponentQuirks(
    ...
    // MStar Android Patch Begin
    // Fixed for AC ON/OFF Auto Test for OMX.google.vorbis.decoder
    if (list->codecHasQuirk(
                index, "requires-flush-before-shutdown")) {
        quirks |= kRequiresFlushBeforeShutdown;
        ALOGV("kRequiresFlushBeforeShutdown");
    }
...
sp<MediaSource> OMXCodec::Create(
    ...
    // MStar Android Patch Begin
    int32_t requiresSecureBuffers, CameraId = -1;
    sp<MetaData> metadata = source->getFormat();
    if (metadata->findInt32(
    ...
    if (metadata->findInt32(kKeyCameraId, &CameraId)) {
        if ((CameraId >= 0) && (CameraId < MAX_CAMERAS)) {// 0 1 2: WebCam, 3: VE, 4: Dwin, 5: XCDip;
            //LOGD("OMXCodec::Create  success=========CameraId %d", CameraId);
            flags |= kMediaSourceCamera;
        }
    }
    // MStar Android Patch End
    ...
    for (size_t i = 0; i < matchingCodecs.size(); ++i) {
        ...
        if (err == OK) {
            ...
            // MStar Android Patch Begin
            int32_t msTeePath = 0;
            if (metadata->findInt32(kKeyMsTeePath, &msTeePath)) {
                //borrow priority config struture for TEE configuration
                char value[PROPERTY_VALUE_MAX];
                if (property_get("ms.noTee", value, NULL) && (!strcmp("1", value)))
                    msTeePath = 0; //force no tee
                OMX_PRIORITYMGMTTYPE teeParams;
                InitOMXParams(&teeParams);

                //inform MStar h/w component to send data thru TEE or not
                ALOGW("%s : set MStar TEE Path [%s]", componentName, msTeePath ? "ON" : "OFF");
                teeParams.nGroupPriority = msTeePath;
                if (OK != omx->setParameter(node, OMX_IndexMstarTeePath, &teeParams, sizeof(teeParams)))
                    ALOGW("%s : set OMX_IndexMstarTeePath failed!!", componentName);
            }
            // MStar Android Patch End
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
        ...
        if (meta->findData(kKeyESDS, &type, &data, &size)) {
            ...
            addCodecSpecificData(data, size);
        // MStar Android Patch Begin
        } else if (meta->findData(kKeyMsDecConfigData, &type, &data, &size)) {
            addCodecSpecificData(data, size);
        // MStar Android Patch End
        }
    }
    ...
    // MStar Android Patch Begin
#if ENABLE_AUDIO_HW_DECODE
    } else if(!strcasecmp(MEDIA_MIMETYPE_AUDIO_ADPCM, mMIME)
            ||!strcasecmp(MEDIA_MIMETYPE_AUDIO_G711_ALAW, mMIME)
            || !strcasecmp(MEDIA_MIMETYPE_AUDIO_G711_MLAW, mMIME)) {
        size_t size;
        uint32_t type;
        const void* data;
        if (!meta->findData(kKeyMsAdpcmProfile,&type,&data,&size)) {
            return BAD_VALUE;
        }
        if (size != sizeof(OMX_AUDIO_PARAM_ADPCMTYPE)) {
            return BAD_VALUE;
        }
        status_t err = setADPCMFormat((OMX_AUDIO_PARAM_ADPCMTYPE*)data);
        if (err != OK) {
            return err;
        }
    } else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_LPCM, mMIME)) {
        int32_t audFmt, bitsPerSample, numChannels, sampleRate;
        ...
        if(!meta->findInt32(kKeyMsPcmAudFmt, &audFmt))
            CODEC_LOGE("cannot find \"audFmt\"");
        else
            CODEC_LOGE("audFmt[%d]",audFmt);
        if(!meta->findInt32(kKeyChannelCount, &numChannels))
            CODEC_LOGE("cannot find \"numChannels\"");
        else
            CODEC_LOGE("numChannels[%d]",numChannels);
        if(!meta->findInt32(kKeySampleRate, &sampleRate))
            CODEC_LOGE("cannot find \"sampleRate\"");
        else
            CODEC_LOGE("sampleRate[%d]",sampleRate);
        if(!meta->findInt32(kKeyMsBitsPerSample, &bitsPerSample))
            CODEC_LOGE("cannot find \"bitsPerSample\"");
        else
            CODEC_LOGE("bitsPerSample[%d]",bitsPerSample);

        profile.ePCMMode = OMX_AUDIO_PCMModeLinear;
        profile.nChannels = numChannels;
        profile.nBitPerSample = bitsPerSample;
        profile.nSamplingRate = sampleRate;

        //mst player definition
        #define WAVE_FORMAT_PCM_TWOS 0x7655 // signed, big endian format
        #define WAVE_FORMAT_PCM_SOWT 0x7656 // signed, little endian format

        if (WAVE_FORMAT_PCM_TWOS == audFmt)
            profile.eEndian = OMX_EndianBig;
        else
            profile.eEndian = OMX_EndianLittle;

        err = mOMX->setParameter(mNode, OMX_IndexParamAudioLpcm, &profile, sizeof(profile));
        if (err != OK) {
            CODEC_LOGE("setParameter('OMX_IndexParamAudioLpcm') failed (err = 0x%x)", err);
            return err;
        }
    }
#else
...
#endif
    else if(!strcasecmp(MEDIA_MIMETYPE_AUDIO_COOK, mMIME)) {
        size_t size;
        uint32_t type;
        const void *data;
        if (!meta->findData(kKeyMsRaProfile,&type,&data,&size)) {
            return BAD_VALUE;
        }
        if (size != sizeof(OMX_AUDIO_PARAM_RATYPE)) {
            return BAD_VALUE;
        }
        status_t err = setRAFormat((OMX_AUDIO_PARAM_RATYPE*)data);
        if (err != OK) {
            return err;
        }
    } else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_WMA, mMIME)) {
        int32_t s32NumChannels, s32SampleRate, s32Profile, s32BlockAlign, s32EncodeOptions, s32AdvancedEncodeOptions;
        int32_t s32BitsPerSample, s32ChannelMask;
        OMX_AUDIO_PARAM_WMATYPE stWmaInfo;

        memset(&stWmaInfo, 0, sizeof(OMX_AUDIO_PARAM_WMATYPE));

        stWmaInfo.eFormat = OMX_AUDIO_WMAFormat9;

        if (!meta->findInt32(kKeyBitRate, &bitRate)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyChannelCount, &s32NumChannels)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeySampleRate, &s32SampleRate)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyMsWMAProfile, &s32Profile)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyMsWMABlockAlign, &s32BlockAlign)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyMsWMAEncodeOptions, &s32EncodeOptions)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyMsBitsPerSample, &s32BitsPerSample)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyMsWMAChannelMask, &s32ChannelMask)) {
            return BAD_VALUE;
        }
        if (!meta->findInt32(kKeyMsWMAAdvancedEncodeOptions, &s32AdvancedEncodeOptions)) {
            return BAD_VALUE;
        }
        stWmaInfo.nBitRate = bitRate;
        stWmaInfo.nChannels = (OMX_U16)s32NumChannels;
        stWmaInfo.nSamplingRate = (OMX_U32)s32SampleRate;
        stWmaInfo.eProfile = (OMX_AUDIO_WMAPROFILETYPE)s32Profile;
        stWmaInfo.nBlockAlign = (OMX_U16)s32BlockAlign;
        stWmaInfo.nEncodeOptions = (OMX_U16)s32EncodeOptions;
        stWmaInfo.nSuperBlockAlign = (OMX_U32)stWmaInfo.nBlockAlign;
        stWmaInfo.nBitsPerSample = (OMX_U16)s32BitsPerSample;
        stWmaInfo.nAdvancedEncodeOpt = (OMX_U32)s32AdvancedEncodeOptions;
        stWmaInfo.nChannelMask = (OMX_U32)s32ChannelMask;
        status_t err = setWMAFormat(&stWmaInfo);
        if (err != OK) {
            return err;
        }
    // MStar Android Patch End
    ...
    if (!strncasecmp(mMIME, "video/", 6)) {
        ...
        if (mIsEncoder) {
            ...
            if (mFlags & kIgnoreAvcFrameGap)
            {
                temp = 1;
                mOMX->setParameter(mNode, OMX_IndexMstarIgnoreAvcFrameGap, &temp, sizeof(temp));
            }

            if (mFlags & kLiveStreamingPlayback)
            {
                temp = 1;
                mOMX->setParameter(mNode, OMX_IndexMstarLiveStreamingPlayback, &temp, sizeof(temp));
            }

            if (mFlags & kForceDecode)
            {
                temp = 1;
                mOMX->setParameter(mNode, OMX_IndexMstarForceDecode, &temp, sizeof(temp));
            }

            if (meta->findInt32(kKeyMsTimeStampMode, &temp)) {
                if (temp == 1) {
                    mOMX->setParameter(mNode, OMX_IndexMstarInputPTSMode, &temp, sizeof(temp));
                }
            }

            if (meta->findInt32(kKeyMs4K1KLRMode, &temp)) {
                if (temp == 1) {
                    mOMX->setParameter(mNode, OMX_IndexMstar4K1KLRMode, &temp, sizeof(temp));
                }
            }

            if (meta->findInt32(kKeyMsFrameRateNum, &temp)) {
                mOMX->setParameter(mNode, OMX_IndexMstarFrameRateNum, &temp, sizeof(temp));
            }

            if (meta->findInt32(kKeyMsFrameRateDen, &temp)) {
                mOMX->setParameter(mNode, OMX_IndexMstarFrameRateDen, &temp, sizeof(temp));
            }

            if (meta->findInt32(kKeyMsMstFrameRateNum, &temp)) {
                mOMX->setParameter(mNode, OMX_IndexMstarMstFrameRateNum, &temp, sizeof(temp));
            }

            if (meta->findInt32(kKeyMsMstFrameRateDen, &temp)) {
                mOMX->setParameter(mNode, OMX_IndexMstarMstFrameRateDen, &temp, sizeof(temp));
            }
            // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    int isHardwareRender = 0;
    status_t err = OK;
    if (mNativeWindow != NULL && !strncmp(mComponentName, "OMX.MS.", 7) && mIsVideo && !mIsEncoder)
        err = mNativeWindow->query(
                mNativeWindow.get(), NATIVE_WINDOW_HARDWARE_RENDER,
                &isHardwareRender);
    if (((mFlags & kClientNeedsFramebuffer)||(!(mFlags & kMstarSpecificFlag) && err == OK && isHardwareRender == 0))
            && !strncmp(mComponentName, "OMX.MS.", 7) && mIsVideo && !mIsEncoder) {
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    initOutputFormat(meta); // After ThumbnailMode, format will be changed!
    // MStar Android Patch End
...
static size_t getFrameSize(
    ...
    switch (colorFormat) {
        ...
        */
        ...
        // MStar Android Patch Begin
        // Bypass Encoded ES from device
        case OMX_COLOR_FormatEncodedAVC:
            return 1024; //width * height + 1024;
        // MStar Android Patch End
...
void OMXCodec::setVideoInputFormat(
    ...
    // MStar Android Patch Begin
    // Bypass Encoded ES from device
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_BYPASS, mime)) {
        bool success = meta->findCString(kKeySubMIMEType, &mime);
        CHECK(success);
        ALOGI("video format for bypass mime type '%s'.", mime);
    }
    // MStar Android Patch End
...
// MStar Android Patch Begin
// update encoding settings runtime
status_t OMXCodec::updateBitrate(int32_t bitRate) {
    OMX_VIDEO_CONFIG_BITRATETYPE bitrateConfig;
    InitOMXParams(&bitrateConfig);
    bitrateConfig.nPortIndex = kPortIndexOutput;

    bitrateConfig.nEncodeBitrate = bitRate;

    status_t err = mOMX->setConfig(
            mNode, OMX_IndexConfigVideoBitrate,
            &bitrateConfig, sizeof(bitrateConfig));
    CHECK_EQ(err, (status_t)OK);
    return OK;
}
// MStar Android Patch End
...
status_t OMXCodec::setVideoOutputFormat(
    ...
    // MStar Android Patch Begin
        ...
        compressionFormat = OMX_VIDEO_CodingDIVX311;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_DIVX4, mime)) {
        compressionFormat = OMX_VIDEO_CodingDIVX412;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_FLV, mime)) {
        compressionFormat = OMX_VIDEO_CodingFLV;
    ...
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVS, mime)) {
        compressionFormat = OMX_VIDEO_CodingAVS;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MVC, mime)) {
...
void OMXCodec::setComponentRole(
    ...
    static const MimeToRole kMimeToRole[] = {
        ...
        { MEDIA_MIMETYPE_AUDIO_DTS_LBR,
            "audio_decoder.dtslbr", "audio_encoder.dtslbr" },
...
status_t OMXCodec::init() {
    ...
    // MStar Android Patch Begin
    return mState == ERROR ? mFinalStatus : OK;
    // MStar Android Patch End
}
...
// MStar Android Patch Begin
static OMX_U32 translateErrorCode(OMX_U32 u32InErrCode)
{
    OMX_U32 u32OutErrCode = 0;
    switch (u32InErrCode) {
        case OMX_ErrorMstarVideoFormatUnsupported:
        case OMX_ErrorMstarVideoResolutionUnsupported:
            u32OutErrCode = ERROR_VIDEO_UNSUPPORT;
            break;
        case OMX_ErrorInsufficientResources:
            u32OutErrCode = ERROR_OUT_OF_MEMORY;
            break;
        case OMX_ErrorResourcesLost:
            u32OutErrCode = ERROR_VIDEO_RESOURCE_LOST;
            break;
        case OMX_ErrorMstarVideoContinousSkipFrames:
            u32OutErrCode = ERROR_VIDEO_SKIP_TOO_MUCH;
            break;
        case OMX_ErrorMstarAudioFormatUnsupported:
            u32OutErrCode = ERROR_AUDIO_UNSUPPORT;
            break;
        case OMX_ErrorTimeout:
            u32OutErrCode = ERROR_TIMEOUT;
            break;
        default:
            break;
    }
    return u32OutErrCode;
}
// MStar Android Patch End
...
void OMXCodec::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        ...
        case OMX_EventError:
        {
            ...
            // MStar Android Patch Begin
            data1 = translateErrorCode(data1);
            if (data1 != 0) {
                mFinalStatus = data1;
            }
            // MStar Android Patch End
        ...
        case OMX_EventPortSettingsChanged:
        {
            ...
            if (data2 == 0 || data2 == OMX_IndexParamPortDefinition) {
                ...
                // MStar Android Patch Begin
                /* If user issue OMXCodec stop, OMXCodec state is set to EXECUTING_TO_IDLE and in/out ports is shutting down.
                   Thus, we should ignore PortSettingsChanged event */
    ...
    switch (newState) {
        case OMX_StateIdle:
        {
            ...
            if (mState == LOADED_TO_IDLE) {
                ...
                // MStar Android Patch Begin
                else if (mNativeWindow != NULL && !strncmp(mComponentName, "OMX.MS.", 7) && !mIsEncoder && mIsVideo) {
                    pushBlankBuffersToNativeWindow();
                }
                // MStar Android Patch End
...
// MStar Android Patch Begin
// In place decryption of a protected sample.
// The buffer must contain one complete Common Encryption Sample.
// The crypto must contain all of the key, iv, encrypted and
// plain sizes.
// Only the Counter Mode is implemented.
bool OMXCodec::DecryptBuffer(sp<ABuffer> buffer, sp<MetaData> crypto) {

#ifdef BUILD_WITH_MARLIN
    uint32_t type;
    const void *key, *iv, *enc_sizes, *plain_sizes;
    uint32_t key_len, iv_len, enc_sizes_len, plain_sizes_len;

    CHECK(crypto->findData(kKeyCryptoKey, &type, &key, &key_len));
    CHECK(crypto->findData(kKeyCryptoIV,  &type, &iv,  &iv_len));
    CHECK(crypto->findData(kKeyEncryptedSizes, &type,
                           &enc_sizes, &enc_sizes_len));
    CHECK(crypto->findData(kKeyPlainSizes, &type,
                           &plain_sizes, &plain_sizes_len));

    CHECK(enc_sizes_len == plain_sizes_len);

    if (mBlockCipherFactory == NULL) {
        CHECK_EQ(WSB_SkbAp4BlockCipherFactory_Create(mBlockCipherFactory),
                 WSB_SUCCESS);
    }

    // Check whether we have new key
    if (mDecrypter == NULL || mKey == NULL ||
        mKey->size() != key_len ||
        memcmp(mKey->data(), key, key_len) != 0)
    {
        // Update the key and the decrypter
        ALOGI("Updating the CENC decrypter");
        mKey = new ABuffer(key_len);
        memcpy(mKey->data(), key, key_len);
        mKey->setRange(0, key_len);
        delete mDecrypter;
        CHECK_EQ(AP4_CencSingleSampleDecrypter::Create(AP4_CENC_ALGORITHM_ID_CTR,
                                                       mKey->data(),
                                                       mKey->size(),
                                                       mBlockCipherFactory,
                                                       mDecrypter), AP4_SUCCESS);
    }

    AP4_UI16* plain_sizes_ui16 =
        ReformatArray<size_t, AP4_UI16>((const size_t*)plain_sizes,
                                        plain_sizes_len);
    AP4_UI32* encrypted_sizes_ui32 =
        ReformatArray<size_t, AP4_UI32>((const size_t*)enc_sizes,
                                        enc_sizes_len);

    AP4_Result result;
    AP4_DataBuffer ap4_in_buffer;
    ap4_in_buffer.SetBuffer(buffer->data(), buffer->size());
    ap4_in_buffer.SetDataSize(buffer->size());
    AP4_DataBuffer ap4_out_buffer;
    result = mDecrypter->DecryptSampleData(ap4_in_buffer, ap4_out_buffer,
                                           (const AP4_UI08*)iv,
                                           enc_sizes_len / sizeof(size_t),
                                           plain_sizes_ui16,
                                           encrypted_sizes_ui32);

    delete [] plain_sizes_ui16;
    delete [] encrypted_sizes_ui32;

    if (result == AP4_SUCCESS) {
        // Copy out buffer back to in buffer
        memcpy(buffer->data(), ap4_out_buffer.GetData(), buffer->size());
    }
...
/*
    add crypto info to omx extradata field so it can be brought to TEE via OMX
    buffer header.
    data will be arranged as:
    [payload] + [mstar extradata header] + [omx extradta header] + [cryptoInfo]

*/
#define roundUpTo32BitBoundary(X) ((X) + 3) / 4 * 4
bool OMXCodec::packCryptoInfo(sp<ABuffer> buffer, sp<MetaData> crypto, size_t padding_size) {

#ifdef BUILD_WITH_MARLIN
    uint32_t type;
    const void *key, *iv, *enc_sizes, *plain_sizes;
    uint32_t key_len, iv_len, enc_sizes_len, plain_sizes_len;
    MS_EXTRA_DATA_HEADER *msExtra;
    ...
    const AP4_UI16 *plain_sizes_ui16 =
        ReformatArray<size_t, AP4_UI16>((const size_t*)plain_sizes,
                                        plain_sizes_len);
    const AP4_UI32 *encrypted_sizes_ui32 =
        ReformatArray<size_t, AP4_UI32>((const size_t*)enc_sizes,
                                        enc_sizes_len);
    uint32_t numSubSample = enc_sizes_len / sizeof(size_t);
    CHECK(numSubSample <= MARLIN_MAX_SUBSAMPLE_NUM);

    CryptoInfo* cryptoInfo = CryptoInfo::newCryptoInfo(1); //already reserved MARLIN_MAX_SUBSAMPLE_NUM subsample space
    cryptoInfo->mNumSubSamples = numSubSample;

    cryptoInfo->mMode = CryptoInfo::kMode_AES_MARLIN;
    cryptoInfo->ext_key_size = key_len;
    CHECK(EXTENDED_KEY_SIZE >= key_len);
    if (key_len <= 16) {
        // to be backward compatible with widevine/playready struct
        memcpy(&cryptoInfo->mKey[0], key, key_len);
    }
    memcpy(&cryptoInfo->ext_key[0], key, key_len);
    memcpy(cryptoInfo->mIv, iv, iv_len);
    cryptoInfo->mSubSamples[0].mNumBytesOfClearData = plain_sizes_ui16[0] + padding_size;
    cryptoInfo->mSubSamples[0].mNumBytesOfEncryptedData = encrypted_sizes_ui32[0];
    for (int i = 1; i < numSubSample; i++) {
        cryptoInfo->mSubSamples[i].mNumBytesOfClearData = plain_sizes_ui16[i];
        cryptoInfo->mSubSamples[i].mNumBytesOfEncryptedData = encrypted_sizes_ui32[i];
    }

    //TODO: specify DrmId here if necessary
    //uint8_t testMarlinDrmId[] = {'m','s','t','a','r',' ','m','a','r','l','i','n','D','r','m',0};
    //memcpy(&cryptoInfo->mDrmId[0], &testMarlinDrmId[0], 16);

    //add extra data field
    size_t cryptoInfoSize = sizeof(CryptoInfo); //already leave 8 additional subsample space for marlin
    CHECK(MS_EXTRA_DATA_SIZE >= cryptoInfoSize);
    size_t offsetToAddExtraData = roundUpTo32BitBoundary(buffer->size());

    msExtra = reinterpret_cast<MS_EXTRA_DATA_HEADER *>(
                    reinterpret_cast<OMX_U8 *>(buffer->data()) + offsetToAddExtraData);

    msExtra->actualSize = buffer->size();

    OMX_OTHER_EXTRADATATYPE *omxExtraData =
            reinterpret_cast<OMX_OTHER_EXTRADATATYPE *>(
                    reinterpret_cast<OMX_U8 *>(msExtra) + MS_EXTRA_DATA_HEADER_SIZE);

    size_t omxExtraDataSize = roundUpTo32BitBoundary(
            (omxExtraData->data + cryptoInfoSize) - reinterpret_cast<OMX_U8 *>(omxExtraData));

    InitOMXParams(omxExtraData);
    omxExtraData->nSize = omxExtraDataSize;
    omxExtraData->nPortIndex = OMX_ALL;
    omxExtraData->eType = static_cast<OMX_EXTRADATATYPE>(OMX_ExtraDataCryptoInfo);
    omxExtraData->nDataSize = cryptoInfoSize;

    memcpy(omxExtraData->data, (void *)cryptoInfo, cryptoInfoSize);
    buffer->meta()->setInt32("extra-data", true);
    buffer->setRange(0, offsetToAddExtraData + MS_EXTRA_DATA_SIZE);

    CryptoInfo::deleteCryptoInfo(cryptoInfo);
...
bool OMXCodec::drainInputBuffer(BufferInfo *info) {
    ...
    if (mCodecSpecificDataIndex < mCodecSpecificData.size()) {
        ...
        // MStar Android Patch Begin
        if ((!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mMIME) || !strcasecmp(MEDIA_MIMETYPE_VIDEO_MVC, mMIME))
    ...
    int32_t n = 0;
    // MStar Android Patch Begin
    int64_t spdif_pts_exist=0, spdif_pts;
    bool hw_tee_path = false;
    sp<ABuffer> abuffer;
...
// MStar Android Patch Begin
#ifdef BUILD_WITH_MARLIN
            if (err == OK && srcBuffer->getABuffer().get()) {
                sp<ABuffer> header;
                bool have_header;
                int64_t pts;

                abuffer = srcBuffer->getABuffer();
                have_header = abuffer->meta()->findBuffer("wasabi-sample-header", &header);

                if (have_header) {
                    // Assume the buffer won't grow by more than 4000?
                    // bytes after NAL units get prefixed by 0 0 0 1
                    // FIXME: This should really be the Codec's
                    // secure buffer and this decryption and NAL unit
                    // prefixing should happen in the secure memory!

                    uint32_t clear_buffer_size =
                        abuffer->size() + 4000 + header->size();

                    sp<ABuffer> clear_buffer = new ABuffer(clear_buffer_size);

                    CHECK(abuffer->meta()->findInt64("timeUs", &pts));
                    CODEC_LOGV("pts: %lld, abuffer size: %d, header size: %d", pts, abuffer->size(), header->size());

                    // Copy the header to the clear_buffer
                    memcpy(clear_buffer->data(), header->data(), header->size());
                    clear_buffer->setRange(0, header->size());

                    sp<RefBase> obj;
                    if (abuffer->meta()->findObject("cenc-crypto", &obj)) {
                        #ifdef DEBUG_MARLIN_LEVEL2
                        char value[PROPERTY_VALUE_MAX];
                        if (property_get("ms.marlin.swdec", value, NULL) && (!strcmp("1", value))) {
                            CHECK(DecryptBuffer(abuffer, static_cast<MetaData*>(obj.get())));
                        } else
                        #endif
                            hw_tee_path = true;
                    }

                    int32_t nalu_len_size;
                    if (abuffer->meta()->findInt32("nalu-length-size", &nalu_len_size)) {
                        // Prefix NAL units here
                        uint32_t prefixed_len;
                        if (WSB_PrefixAvcNalus(nalu_len_size,
                                               abuffer->data(),
                                               abuffer->size(),
                                               clear_buffer->data() + clear_buffer->size(),
                                               clear_buffer->capacity() - clear_buffer->size(),
                                               &prefixed_len) != OK)
                        {
                            ALOGE("Fail to process NAL units");
                        }
                        CODEC_LOGV("Video frame of %u + %u bytes (original nalu_len_size[%d])",
                              clear_buffer->size(), prefixed_len, nalu_len_size);
                        clear_buffer->setRange(0, clear_buffer->size() + prefixed_len);
                    } else {
                        CODEC_LOGV("Audio frame of %u + %u bytes",
                              clear_buffer->size(), abuffer->size());
                        memcpy(clear_buffer->data() + clear_buffer->size(),
                               abuffer->data(), abuffer->size());
                        clear_buffer->setRange(0, clear_buffer->size() + abuffer->size());
                    }

                    if (hw_tee_path) {
                        //pad omx extradata here
                        packCryptoInfo(clear_buffer, static_cast<MetaData*>(obj.get()), header->size());
                    }

                    abuffer = clear_buffer;
                    mbuffer = new MediaBuffer(abuffer);
                    mbuffer->meta_data()->setInt64(kKeyTime, pts);
...
// MStar Android Patch End
        ...
        if (err != OK) {
            // MStar Android Patch Begin
            if (mIsEncoder && (mFlags&kMediaSourceCamera) && mInitialBufferSubmit) {
                CODEC_LOGE("omxcodec draininputbuffer for init return %lld",systemTime());
                return false;
            }

            if (mIsEncoder && (mFlags & kMediaSourceCamera))
                break;
            // MStar Android Patch End
            signalEOS = true;
            mFinalStatus = err;
        ...
        if (srcBuffer->range_length() > remainingBytes) {
            srcBuffer->set_range(0, remainingBytes);
        }
        // MStar Android Patch End
        ...
        {
            spdif_pts_exist = srcBuffer->meta_data()->findInt64(kKeyMsSpdifPts, &spdif_pts);
        }
        // MStar Android Patch End
        ...
        // MStar Android Patch Begin
        if (mIsEncoder && mIsVideo) {
            CHECK(lastBufferTimeUs >= 0);// Note: mstplayer may output -1 PTS for decoder
    ...
    // MStar Android Patch Begin
    //set spdif time diff info for audio DSP sync.
#if 0
    if (spdif_pts_exist)
    {
        CODEC_LOGV("spdif_pts[%ld]",spdif_pts);
        mOMX->setParameter(mNode, OMX_IndexMstarSPDIFOuputPTS, &spdif_pts, sizeof(spdif_pts));
    }
#endif
    // MStar Android Patch End
...
status_t OMXCodec::waitForBufferFilled_l() {
    ...
    if (mIsEncoder) {
        ...
        // for video encoding.
        if (mFlags&kMediaSourceCamera) { // camera source
            int32_t CameraId = -1;
            int64_t camera_source_timeou_ns = 3000000000LL;

            mSource->getFormat()->findInt32(kKeyCameraId, &CameraId);
            if(CameraId >= 3)
                camera_source_timeou_ns = 100000000LL;
            err = mBufferFilled.waitRelative(mLock, camera_source_timeou_ns);
            if (err != OK)
                CODEC_LOGE("OMXCodec wait output buffer time out (%lld ns camerasource)", camera_source_timeou_ns);
            return err;
        }
        else { // not camera source
...
void OMXCodec::setAMRFormat(bool isWAMR, int32_t bitRate) {
    ...
    if (mIsEncoder) {
        ...
        // MStar Android Patch Begin
        // native source hard-codes sampleRate@8KHz on bothe nb and wb
        //setRawAudioFormat(kPortIndexInput, sampleRate, numChannels);
        if (isWAMR){
            setRawAudioFormat(kPortIndexInput, 16000, numChannels);
        } else {
            setRawAudioFormat(kPortIndexInput, sampleRate, numChannels);
        }
        // MStar Android Patch End
...
// MStar Android Patch Begin
status_t OMXCodec::setRAFormat(OMX_AUDIO_PARAM_RATYPE *pstRaInfo)
{
    if (mIsEncoder) {
        CODEC_LOGE("not support encode");
        return UNKNOWN_ERROR;
    } else {
        ...
        status_t err = mOMX->getParameter(
            mNode, OMX_IndexParamAudioRa, &profile, sizeof(profile));
        if (err!=OK) {
            CODEC_LOGE("getParameter('OMX_IndexParamAudioRa') failed (err = %d)", err);
            return err;
        }
        profile.nNumCodecs = pstRaInfo->nNumCodecs;
        profile.nSamplePerFrame = pstRaInfo->nSamplePerFrame;
        profile.nSamplingRate = pstRaInfo->nSamplingRate;
        profile.eFormat = pstRaInfo->eFormat;
        profile.nFramesPerBlock = pstRaInfo->nFramesPerBlock;

        for (int j=0; j<profile.nNumCodecs; j++) {
            profile.nChannels[j] = pstRaInfo->nChannels[j];
            profile.nNumRegions[j] = pstRaInfo->nNumRegions[j];
            profile.nCouplingStartRegion[j] = pstRaInfo->nCouplingStartRegion[j];
            profile.nCouplingQuantBits[j] = pstRaInfo->nCouplingQuantBits[j];
            profile.nBitsPerFrame[j] = pstRaInfo->nBitsPerFrame[j];
        }

        err = mOMX->setParameter(mNode, OMX_IndexParamAudioRa, &profile, sizeof(profile));
        if (err != OK) {
            CODEC_LOGE("setParameter('OMX_IndexParamAudioRa') failed (err = %d)", err);
            return err;
        }
    }
    return OK;
...
status_t OMXCodec::setADPCMFormat(OMX_AUDIO_PARAM_ADPCMTYPE *pstAdpcmInfo)
{
    if (mIsEncoder) {
        CODEC_LOGE("not support encode");
        return UNKNOWN_ERROR;
        ...
        status_t err = mOMX->getParameter(mNode, OMX_IndexParamAudioAdpcm, &profile, sizeof(profile));
        if (err!=OK) {
            CODEC_LOGE("getParameter('OMX_IndexParamAudioAdpcm') failed (err = %d)", err);
            return err;
        }

        profile.eType = pstAdpcmInfo->eType;
        profile.nBitsPerSample = pstAdpcmInfo->nBitsPerSample;
        profile.nBlockSize = pstAdpcmInfo->nBlockSize;
        profile.nChannels = pstAdpcmInfo->nChannels;
        profile.nSampleRate = pstAdpcmInfo->nSampleRate;

        err = mOMX->setParameter(mNode, OMX_IndexParamAudioAdpcm, &profile, sizeof(profile));
        if (err != OK) {
            CODEC_LOGE("setParameter('OMX_IndexParamAudioAdpcm') failed (err = %d)", err);
            return err;
        }
    }
    return OK;
...
status_t OMXCodec::setWMAFormat(OMX_AUDIO_PARAM_WMATYPE *pstWmaInfo)
{
    if (pstWmaInfo->nChannels > 6) {
        CODEC_LOGE("Number of channels: (%d),out of support \n", pstWmaInfo->nChannels);
        return UNKNOWN_ERROR;
    }
    ...
    if (mIsEncoder) {
        ...
        status_t err = mOMX->getParameter(mNode, OMX_IndexParamAudioWma, &profile, sizeof(profile));
        if (err != OK) {
            CODEC_LOGE("getParameter('OMX_IndexParamAudioWma') failed (err = %d)", err);
            return err;
        }

        profile.nChannels = pstWmaInfo->nChannels;
        profile.nSamplingRate = pstWmaInfo->nSamplingRate;
        profile.nBitRate = pstWmaInfo->nBitRate;
        profile.eFormat = pstWmaInfo->eFormat;
        profile.eProfile = pstWmaInfo->eProfile;
        profile.nBlockAlign = pstWmaInfo->nBlockAlign;
        profile.nSuperBlockAlign = pstWmaInfo->nSuperBlockAlign;
        profile.nEncodeOptions = pstWmaInfo->nEncodeOptions;
        profile.nBitsPerSample = pstWmaInfo->nBitsPerSample;
        profile.nAdvancedEncodeOpt = pstWmaInfo->nAdvancedEncodeOpt;

        err = mOMX->setParameter(mNode, OMX_IndexParamAudioWma, &profile, sizeof(profile));
        if (err != OK) {
            CODEC_LOGE("setParameter('OMX_IndexParamAudioWma') failed (err = %d)", err);
            return err;
        }
    }
...
status_t OMXCodec::start(MetaData *meta) {
    ...
    if (err != OK)
    {
        CODEC_LOGE("omxcodec failed to start: %d", err);
        mSource->stop();
    }
    return err;
    // MStar Android Patch End
...
sp<MetaData> OMXCodec::getFormat() {
    // MStar Android Patch Begin
    if (!strncmp(mComponentName, "OMX.MS.", 7) && !mIsEncoder && mIsVideo && (mFlags & kMstarSpecificFlag)) {
        int32_t temp;

        if (mOutputFormat->findInt32(kKeyFrameRate, &temp)) {
            if (temp == 0) {
                ALOGD("getFormat Begin");
                status_t err;
                OMX_PARAM_PORTDEFINITIONTYPE def;
                OMX_VIDEO_PORTDEFINITIONTYPE *video_def = &def.format.video;
                ...
                temp = 100; // 100 * 10000 us ==> 1s

                while (temp--) {
                    if (mState != EXECUTING && mState != RECONFIGURING) {
                        mOutputFormat->setInt32(kKeyMsErrorCode, mFinalStatus);
                        break;
                    }
                    ...
                    { // mutex start
                        ...
                        if (video_def->xFramerate != 0) {

                            mOutputFormat->setInt32(kKeyWidth, video_def->nFrameWidth);
                            mOutputFormat->setInt32(kKeyHeight, video_def->nFrameHeight);
                            mOutputFormat->setInt32(kKeyFrameRate, video_def->xFramerate);

                            MSTAR_OMX_VIDEO_DECODER_INFO info;
                            err = mOMX->getParameter(
                                    mNode, OMX_IndexMstarVideoDecoderInfo, &info, sizeof(info));

                            CHECK_EQ(err, (status_t)OK);

                            mOutputFormat->setInt32(kKeyMsInterlace, info.bInterlace);
                            mOutputFormat->setInt32(kKeyMsCodecType, info.u32CodecType);
                            mOutputFormat->setInt32(kKeyMsVopFrameRate, info.u32VopFrameRate);
                            mOutputFormat->setInt32(kKeyMsAspectWidth, info.u32AspectWidth);
                            mOutputFormat->setInt32(kKeyMsAspectHeight, info.u32AspectHeight);
                            mOutputFormat->setInt32(kKeyMsDisp3DMode, info.u83DMode);
                            mOutputFormat->setInt32(kKeyMsSHMAddr, info.u32SHMAddr);
                            mOutputFormat->setInt32(kKeyMsCropLeft, info.u32CropLeft);
                            mOutputFormat->setInt32(kKeyMsCropRight, info.u32CropRight);
                            mOutputFormat->setInt32(kKeyMsCropTop, info.u32CropTop);
                            mOutputFormat->setInt32(kKeyMsCropBottom, info.u32CropBottom);
                            mOutputFormat->setInt32(kKeyMsDataOnMIU, info.u8DataOnMIU);
                            mOutputFormat->setInt32(kKeyMsCurIndex, info.u8CurIndex);
                            mOutputFormat->setInt32(kKeyMsFrcMode, info.u8FrcMode);
                            mOutputFormat->setInt32(kKeyMsOverlayID, info.u8OverlayID);
                            mOutputFormat->setInt32(kKeyMsErrorCode, translateErrorCode(info.nErrorCode));
                        ...
                        else if (mSignalledEOS && mNoMoreOutputData) { // we must check mNoMoreOutputData to make sure decoder have output all frames and EOS
                            ALOGW("return_EOS, set kKeyMsErrorCode to ERROR_END_OF_STREAM");
                            mOutputFormat->setInt32(kKeyMsErrorCode, ERROR_END_OF_STREAM);
                            break;
                        }

                        mOutputFormat->setInt32(kKeyMsErrorCode, ERROR_CONNECTED_TIMEOUT);
                    } // mutex end
                    usleep(10000);
                }
                ALOGD("getFormat End: %d %d %d", (int) video_def->nFrameWidth, (int) video_def->nFrameHeight, (int) video_def->xFramerate);
            }
        }
    }
    // MStar Android Patch End
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    // MStar Android Patch Begin
    // update encoding settings runtime
    if (options && options->getUpdateParam()) {
        int32_t    bitrate = 0;
        if(mOutputFormat->findInt32(kKeyBitRate, &bitrate)) {
            updateBitrate(bitrate);
            CODEC_LOGI("update encoding parameters bitrate: %d", bitrate);
        }
    }
    // MStar Android Patch End
    ...
    if (mInitialBufferSubmit) {
        // MStar Android Patch Begin
        if (!(mFlags&kMediaSourceCamera)) // not camera source
            mInitialBufferSubmit = false;
        // MStar Android Patch End
        ...
        drainInputBuffers();
        // MStar Android Patch Begin
        if (mInitialBufferSubmit && (mFlags&kMediaSourceCamera)) // camera source
            return UNKNOWN_ERROR;
        // MStar Android Patch End
        ...
        // MStar Android Patch Begin
        if (!strncmp(mComponentName, "OMX.MS.", 7) && !mIsEncoder && mIsVideo && (mFlags & kMstarSpecificFlag) && (mFlags & kMstarInitDecFast))
                return OK;
        // MStar Android Patch End
    }
...
// MStar Android Patch Begin
void OMXCodec::setTrickMode(TrickMode mode) {
    OMX_U32 trick_mode = mode;
    mOMX->setParameter(mNode, OMX_IndexMstarTrickMode, &trick_mode, sizeof(trick_mode));
}
// MStar Android Patch End
...
void OMXCodec::initOutputFormat(const sp<MetaData> &inputFormat) {
    ...
    if (mIsEncoder) {
        ...
        // MStar Android Patch Begin
        //For 3rd party recorder to use setConfig directly
        int32_t current_node_id = *(int32_t*)&mNode;
        mOutputFormat->setInt32(kKeyMstarEncodeNodeId, current_node_id);
        ALOGV("kKeyMstarEncodeNodeId node id = %d.", current_node_id);
        // MStar Android Patch End
    }
    ...
    switch (def.eDomain) {
        ...
        case OMX_PortDomainVideo:
        {
            ...
            // MStar Android Patch Begin
            mOutputFormat->setInt32(kKeyFrameRate, 0);
            // MStar Android Patch End
