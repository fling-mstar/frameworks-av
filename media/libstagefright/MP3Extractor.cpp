...
static int new_bitrate = 0; // some mp3 start with 32K, but follewing with 128K
#define MAX_FRAME_NUM_FOR_CALCULATE_AVERAGE_BITRATE  200
#define ENABLE_CALCULATE_AVERAGE_BITRATE_FOR_VBR    0
// MStar Android Patch End

// MStar Android Patch Begin
#if  ENABLE_CALCULATE_AVERAGE_BITRATE_FOR_VBR
//for some vbr mp3, calculate more frames to get average bitrate.
static bool CalculateAverageBitrate(const sp<DataSource> &source, uint32_t u32FrameSize, uint32_t u32header,
        off64_t inout_pos, uint32_t *pu32AverageBitrate)
{
    off64_t test_pos = inout_pos + u32FrameSize;
    uint32_t u32TotalBitrate = 0;
    int j;

    for (j = 0; j < MAX_FRAME_NUM_FOR_CALCULATE_AVERAGE_BITRATE; ++j) {
        uint8_t tmp[4];
        ...
        if ((test_header & kMask) != (u32header & kMask)) {
            break;
        }
        ...
        if (!GetMPEGAudioFrameSize(
                    test_header, &test_frame_size, &sample_rate, &num_channels, &bitrate)){
            break;
        }

        u32TotalBitrate += bitrate;
    ...
    if((u32TotalBitrate > 0) && (j > 0))
    {
        *pu32AverageBitrate = u32TotalBitrate/j;
        return true;
    }

    *pu32AverageBitrate = 0;
    return false;
}
#endif
// MStar Android Patch End
...
static bool Resync(
    ...
    do {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (bitrate != new_bitrate) // we use bitrate from 4th frame!
                new_bitrate = bitrate;
            // MStar Android Patch End
...
MP3Extractor::MP3Extractor(
    ...
    // MStar Android Patch Begin
    if (new_bitrate && (bitrate!=new_bitrate)) {
        ALOGW("bitrate: %dKbps -> %dKbps \n", bitrate, new_bitrate);
        bitrate = new_bitrate; // use bitrate get from Resync(4th frame!)
    }
    // MStar Android Patch End
    ...
    if(mSeeker == NULL)
    {
        uint32_t u32AverageBitrate = 0;
        //some mp3 is vbr, we calculate average bitrate
        CalculateAverageBitrate(mDataSource, frame_size, header, mFirstFramePos, &u32AverageBitrate);
        if(u32AverageBitrate != 0)
        {
            bitrate = u32AverageBitrate;
            ALOGD("mp3 AverageBitrate:%d\n", u32AverageBitrate);
        }
    }
#endif
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    if ((mSeeker!=NULL) && (durationUs==0)) {
        ALOGI("VBR something wrong, use 128Kbps");
        mMeta->setInt32(kKeyBitRate, 128000);

        off64_t fileSize;
        if (mDataSource->getSize(&fileSize) == OK) {
            durationUs = 8000LL * (fileSize - mFirstFramePos) / 128;
        } else {
            durationUs = -1;
        }
    }
    ...
    if (durationUs >= 0) {
        ...
        off64_t fileSize;
        if (mDataSource->getSize(&fileSize) == OK) {
            bitrate = 8000LL * (fileSize - mFirstFramePos) / durationUs;
            mMeta->setInt32(kKeyBitRate, bitrate * 1000);
            ALOGD(" Set  mp3 Bitrate = %d  again by qingxin.wang 20121017 ",bitrate*1000);
        }
        // MStar Android Patch End
    }
