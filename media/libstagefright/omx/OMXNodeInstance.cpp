...
status_t OMXNodeInstance::useGraphicBuffer2_l(
    ...
    // MStar Android Patch Begin
    def.nVersion.s.nVersionMajor = OMX_VERSION_MAJOR;
    def.nVersion.s.nVersionMinor = OMX_VERSION_MINOR;
    def.nVersion.s.nRevision = OMX_VERSION_REVISION;
    def.nVersion.s.nStep = OMX_VERSION_STEP;
    // MStar Android Patch End
...
status_t OMXNodeInstance::emptyBuffer(
    ...
    // MStar Android Patch Begin
    /* we use pInputPortPrivate to store the address of MS extra data */
    if (header->nFlags & OMX_BUFFERFLAG_EXTRADATA) {
        size_t offset = header->nOffset + header->nFilledLen - MS_EXTRA_DATA_SIZE;
        header->pInputPortPrivate = header->pBuffer + offset ;
    }
    // MStar Android Patch End
