...
// Wasabi includes for secure path decryption
...
status_t ACodec::configureCodec(
    ...
    if (video) {
        if (encoder) {
            ...
            if (!msg->findInt32("width", &width)
                ...
                // MStar Android Patch Begin
                int32_t lowlatency;
                if (msg->findInt32("lowlatency", &lowlatency)) {
                    if (lowlatency) {
                        err = mOMX->setParameter(
                            mNode, OMX_IndexMstarLiveStreamingPlayback, &lowlatency, sizeof(lowlatency));
                    }
                }

                int32_t temp;
                if (msg->findInt32("time-stamp-mode", &temp)) {
                    if (temp == 1) {
                        ALOGV("Set OMX_IndexMstarInputPTSMode %u", temp);
    ...
    // MStar Android Patch Begin
    } else if ((!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_WMAPRO)) ||
               (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_WMA))) {
        ALOGV("configure WMA");
        int32_t numChannels, sampleRate, bitRate, blockAlign,
                eProfile, encodeOpt, bitsPerSample, channelMask, advanceEncodeOpt;
        if (!msg->findInt32("channel-count", &numChannels) ||
            !msg->findInt32("sample-rate", &sampleRate) ||
            !msg->findInt32("bit-rate", &bitRate) ||
            !msg->findInt32("block-align", &blockAlign) ||
            !msg->findInt32("profile", &eProfile) ||
            !msg->findInt32("encode-option", &encodeOpt) ||
            !msg->findInt32("bits-sample", &bitsPerSample) ||
            !msg->findInt32("channel-mask", &channelMask) ||
            !msg->findInt32("advance-encode-option", &advanceEncodeOpt)) {
            err = INVALID_OPERATION;
        } else {
            ...
            profile.nSamplingRate = (OMX_U32)sampleRate;
            profile.nBitRate = bitRate;
            profile.eFormat = OMX_AUDIO_WMAFormat9;
            profile.eProfile = (OMX_AUDIO_WMAPROFILETYPE)eProfile;
            profile.nBlockAlign = (OMX_U16)blockAlign;
            profile.nSuperBlockAlign = (OMX_U32)blockAlign;
            profile.nEncodeOptions = (OMX_U16)encodeOpt;
            profile.nBitsPerSample = (OMX_U16)bitsPerSample;
            profile.nChannelMask = (OMX_U32)channelMask;
            profile.nAdvancedEncodeOpt = (OMX_U32)advanceEncodeOpt;

            ALOGV("******setupWMACodec arg*************");
            ALOGV("profile.nChannels:          %u", profile.nChannels);
            ALOGV("profile.nSamplingRate:      %lu", profile.nSamplingRate);
            ALOGV("profile.nBitRate:           %lu", profile.nBitRate);
            ALOGV("profile.nBlockAlign:        %u", profile.nBlockAlign);
            ALOGV("profile.eFormat:            %u", profile.eFormat);
            ALOGV("profile.eProfile:           %u", profile.eProfile);
            ALOGV("profile.nEncodeOptions:     %u", profile.nEncodeOptions);
            ALOGV("profile.nSuperBlockAlign:   %lu", profile.nSuperBlockAlign);
            ALOGV("profile.nBitsPerSample:     %lu", profile.nBitsPerSample);
            ALOGV("profile.nChannelMask:       %lu", profile.nChannelMask);
            ALOGV("profile.nAdvancedEncodeOpt: %lu", profile.nAdvancedEncodeOpt);
            ALOGV("************************************");
...
static const struct VideoCodingMapEntry {
    ...
    { MEDIA_MIMETYPE_VIDEO_WVC1, OMX_VIDEO_CodingVC1 },
...
// MStar Android Patch Begin
#ifdef BUILD_WITH_MARLIN
        // Wasabi change: 64KB video frame input buffer seems
        // rather small.
        const size_t X = 256 * 1024;
#else
...
bool ACodec::BaseState::onOMXEmptyBufferDone(IOMX::buffer_id bufferID) {
    ...
    // MStar Android Patch Begin
    if (info->mStatus == BufferInfo::OWNED_BY_US) {
        ALOGE("[%s] onOMXEmptyBufferDone : bufferID[%p] may have been returned to us beforehand. skip it!!", mCodec->mComponentName.c_str(), bufferID);
        return true;
    }
    // MStar Android Patch End
...
bool ACodec::DecryptBuffer(sp<ABuffer> buffer, sp<MetaData> crypto)
{
    uint32_t type;
...
void ACodec::BaseState::onInputBufferFilled(const sp<AMessage> &msg) {
    ...
    // MStar Android Patch Begin
    int32_t hasExtraData = false;;
    msg->findInt32("extra-data", &hasExtraData);
    // MStar Android Patch End
...
// MStar Android Patch Begin
                ...
                have_header = buffer->meta()
                    ->findBuffer("wasabi-sample-header", &header);
                ...
                if (have_header) {
                    ...
                    sp<RefBase> obj;
                    if (buffer->meta()
                        ->findObject("cenc-crypto", &obj)) {
                        CHECK(mCodec->DecryptBuffer(buffer, static_cast<MetaData*>(obj.get())));
                    }

                    int32_t nalu_len_size;
                    if (buffer->meta()
                        ->findInt32("nalu-length-size", &nalu_len_size)) {
                        ...
                        ALOGV("Video frame of %u + %u bytes",
                              clear_buffer->size(), prefixed_len);
                        clear_buffer->setRange(0, clear_buffer->size()+prefixed_len);
                    } else {
                        ALOGV("Audio frame of %u + %u bytes",
                              clear_buffer->size(), buffer->size());
                        memcpy(clear_buffer->data()+clear_buffer->size(),
                               buffer->data(), buffer->size());
                        clear_buffer->setRange(0, clear_buffer->size()+buffer->size());
                    }
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    // MStar Android Patch Begin
    if (info->mStatus == BufferInfo::OWNED_BY_US) {
        ALOGE("[%s] onOMXFillBufferDone : bufferID[%p] may have been returned to us beforehand. skip it!!", mCodec->mComponentName.c_str(), bufferID);
        return true;
    }
    // MStar Android Patch End
...
// MStar Android Patch Begin
    // force allocate hardware omx component upon TEE path, so Level 1 security can be achieved
    uint32_t flags = 0;
    int32_t msTeePath = -1;
    msg->findInt32("msTeePath", &msTeePath);

    #ifdef TEE_DEBUG_MODE
    // the debug property will overwrite regular setting
    char value[PROPERTY_VALUE_MAX];
    if (property_get("ms.noTee", value, NULL)) {
        if (!strcmp("1", value)) {
            msTeePath = 0; //bypass tee path
        }
        if (!strcmp("0", value)) {
            msTeePath = 1;
        }
...
// MStar Android Patch End
    ...
    if (msTeePath != (-1)) {
        ...
        ALOGW("%s : set MStar TEE Path [%s]", componentName.c_str(), msTeePath ? "ON" : "OFF");
...
bool ACodec::LoadedState::onConfigureComponent(
    ...
    if (mCodec->mNativeWindow != NULL)
        err = mCodec->mNativeWindow->query(
                mCodec->mNativeWindow.get(), NATIVE_WINDOW_HARDWARE_RENDER,
                &isHardwareRender);
    if (!strncasecmp(mime.c_str(), "video/", 6) && !strncmp(mCodec->mComponentName.c_str(), "OMX.MS.", 7) &&
        ((mCodec->mNativeWindow == NULL) || (err == OK && isHardwareRender == 0))) {
        int32_t encoder;
        if (!msg->findInt32("encoder", &encoder)) {
            encoder = false;
        }
        if (!encoder) {
            OMX_INDEXTYPE index;
            ALOGD("Note: Use Planar Mode");
            status_t err = mCodec->mOMX->getExtensionIndex(mCodec->mNode, "OMX.MS.index.ThumbnailMode", &index);
            if (err == OK) {
                OMX_BOOL enable = OMX_TRUE;
                mCodec->mOMX->setConfig(mCodec->mNode, index, &enable, sizeof(enable));
            }
        }
    }
    // MStar Android Patch End
