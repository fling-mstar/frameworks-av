...
void StagefrightMetadataRetriever::parseMetaData() {
    ...
    for (size_t i = 0; i < numTracks; ++i) {
        ...
        if (trackMeta->findCString(kKeyMIMEType, &mime)) {
            if (!hasAudio && !strncasecmp("audio/", mime, 6)) {
                ...
                // MStar Android Patch Begin
                if (!trackMeta->findInt32(kKeySampleRate, &audioSampleRate)) {
                    audioSampleRate = -1;
                }
                // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    if (hasAudio && audioSampleRate >= 0) {
        sprintf(tmp, "%d", audioSampleRate);
        mMetaData.add(METADATA_KEY_AUDIO_SAMPLE_RATE, String8(tmp));
    }
    // MStar Android Patch End
    ...
    if (numTracks == 1) {
        ...
        // MStar Android Patch Begin
        } else if (!strncasecmp(fileMIME, "video/x-ms-asf", 14)) {
            sp<MetaData> trackMeta = mExtractor->getTrackMetaData(0);
            const char *trackMIME;
            CHECK(trackMeta->findCString(kKeyMIMEType, &trackMIME));
        ...
        } else if (!strcasecmp(fileMIME, "video/x-pn-realvideo")) {
            sp<MetaData> trackMeta = mExtractor->getTrackMetaData(0);
            const char *trackMIME;
            CHECK(trackMeta->findCString(kKeyMIMEType, &trackMIME));
            ...
            if (!strncasecmp("audio/", trackMIME, 6)) {
                mMetaData.add(
                        METADATA_KEY_MIMETYPE, String8("audio/x-pn-realaudio"));
            }
        }
        // MStar Android Patch End
    }
    ...
    // MStar Android Patch Begin
    for (size_t i = 0; i < mMetaData.size(); ++i) {
        ALOGV("key %d : value %s", mMetaData.keyAt(i), mMetaData.valueAt(i).string());
    }
    // MStar Android Patch End
}
