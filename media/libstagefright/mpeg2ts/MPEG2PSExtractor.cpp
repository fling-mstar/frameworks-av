...
MPEG2PSExtractor::MPEG2PSExtractor(const sp<DataSource> &source)
    ...
    // use mstarplay to check format
    // mp3 only in MPEG-PS runs in mstar player
    if (mTracks.size() == 1) {
        const char *mime;
        sp<MetaData> meta = mTracks.valueAt(0)->getFormat();
        meta->findCString(kKeyMIMEType, &mime);
        if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_MPEG))
            meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_UNKNOW);
    }
