...
#if DISABLE_ASSERT
#define CHECK_EQ_I(x,y)   do \
{ \
if ((x) != (y)) \
    return ERROR_MALFORMED;\
} while(0)

#define CHECK_EQ_V(x,y)   do \
{ \
if ((x) != (y)) \
    return;\
} while(0)

#define CHECK_NE_I(x,y)   do \
{ \
if ((x) == (y)) \
    return ERROR_MALFORMED;\
} while(0)

#define CHECK_NE_V(x,y)   do \
{ \
if ((x) == (y)) \
    return;\
} while(0)

#define CHECK_LE_I(x,y)   do \
{ \
if ((x) > (y)) \
    return ERROR_MALFORMED;\
} while(0)

#define CHECK_LT_I(x,y)   do \
{ \
if ((x) >= (y)) \
    return ERROR_MALFORMED;\
} while(0)

#define CHECK_GE_I(x,y)   do \
{ \
if ((x) < (y)) \
    return ERROR_MALFORMED;\
} while(0)

#define CHECK_GT_I(x,y)   do \
{ \
if ((x) <= (y)) \
    return ERROR_MALFORMED;\
} while(0)

#define CHECK_GE_V(x,y)   do \
{ \
if ((x) < (y)) \
    return;\
} while(0)
#else
#define CHECK_EQ_I(x,y) CHECK_EQ((x),(y))
#define CHECK_EQ_V(x,y) CHECK_EQ((x),(y))
#define CHECK_NE_I(x,y) CHECK_NE((x),(y))
#define CHECK_NE_V(x,y) CHECK_NE((x),(y))
#define CHECK_LE_I(x,y) CHECK_LE((x),(y))
#define CHECK_LT_I(x,y) CHECK_LT((x),(y))
#define CHECK_GE_I(x,y) CHECK_GE((x),(y))
#define CHECK_GT_I(x,y) CHECK_GT((x),(y))
#define CHECK_GE_V(x,y) CHECK_GE((x),(y))
#endif
...
// MStar Android Patch Begin
#define ATS_4CC(a,b,c,d) (((a)<<24) | ((b)<<16) | ((c)<<8) | (d))
#define SYS_MIN(a, b)    ((a) < (b) ?  (a) : (b))
// MStar Andriod Patch End
...
status_t ATSParser::Program::parseProgramMap(ABitReader *br) {
    ...
    // MStar Android Patch Begin
    int program_info_length_left = program_info_length;
    if (program_info_length_left > 2) {
        int descriptor_tag;
        int descriptor_length;

        descriptor_tag = br->getBits(8);
        br->skipBits(8);
        program_info_length_left -= 2;
        if (descriptor_tag == 5) {
            descriptor_length = SYS_MIN(4, program_info_length_left);
            uint32_t registration_descriptor = br->getBits(descriptor_length * 8);//get the program registration descriptor,patch for pcm audio
            if (registration_descriptor == ATS_4CC('h','d','m','v')) {
                isHdmv = true;
            }
            program_info_length_left -= descriptor_length;
        }
    }
    br->skipBits(program_info_length_left * 8);  // skip descriptors
    // MStar Android Patch End
    ...
    while (infoBytesRemaining > 0) {
        ...
        // MStar Android Patch Begin
        if (info_bytes_remaining > 0) {
            br->skipBits(info_bytes_remaining * 8);
        }
        // MStar Android Patch End
#endif
    ...
    for (size_t i = 0; i < infos.size(); ++i) {
        ...
        if (index < 0) {
            sp<Stream> stream = new Stream(
                    this, info.mPID, info.mType, PCR_PID, isHdmv);
...
status_t ATSParser::parsePID(
    ...
    if (sectionIndex >= 0) {
        ...
        if (PID == 0) {
            ...
            // MStar Android Patch Begin
            sectionIndex = mPSISections.indexOfKey(PID);
            if (sectionIndex >= 0) {
                section = mPSISections.valueAt(sectionIndex);
            } else {
                return ERROR_MALFORMED;
            }
            section->clear();
...
void ATSParser::parseAdaptationField(ABitReader *br, unsigned PID) {
    ...
    // MStar Android Patch Begin
    // feedTSPacket() function just buffer one packet size
    // We can not skip more than a packet length, or ABitReader assert occurs.
    if (adaptation_field_length > 183) {
        ALOGW("adaption field Length error: %d\n", adaptation_field_length);
        return;
    }
    // MStar Android Patch End
