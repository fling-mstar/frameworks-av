...
// MStar Android Patch Begin
struct BDUPosition { //Bitstream Data Unit
    size_t bduOffset;
    size_t bduSize;
};

sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitVC1() {
    const uint8_t *data = mBuffer->data();

    size_t size = mBuffer->size();
    Vector<BDUPosition> bdus;

    size_t totalSize = 0;

    status_t err;
    const uint8_t *bduStart;

    size_t bduSize;
    bool foundFrame = false;
    while ((err = getNextNALUnit(&data, &size, &bduStart, &bduSize)) == OK) {
        if (bduSize == 0) continue;

        unsigned bduType = bduStart[0] & 0x1f;
        bool flush = false;
        ...
        if (bduType == 0xd) {
            ...
            foundFrame = true;
        } else if (((bduType == 0xf) || (bduType == 0xe)) && foundFrame) {
            // Access unit seqLayer will be associated with the
            // next frame.
            flush = true;
        }

        if (flush) {
            // The access unit will contain all bdu units up to, but excluding
            // the current one, separated by 0x00 0x00 0x01 startcodes.

            size_t auSize = 3 * bdus.size() + totalSize;
            ...
            size_t dstOffset = 0;
            for (size_t i = 0; i < bdus.size(); ++i) {
                const BDUPosition &pos = bdus.itemAt(i);

#if !LOG_NDEBUG
                unsigned bduType = mBuffer->data()[pos.bduOffset] & 0x1f;
                char tmp[128];
                sprintf(tmp, "0x%02x", bduType);
                if (i > 0) {
                    out.append(", ");
                }
                out.append(tmp);
                ...
                memcpy(accessUnit->data() + dstOffset, "\x00\x00\x01", 3);

                memcpy(accessUnit->data() + dstOffset + 3,
                       mBuffer->data() + pos.bduOffset,
                       pos.bduSize);

                dstOffset += pos.bduSize + 3;
            }

            ALOGV("accessUnit contains bdu types %s", out.c_str());

            const BDUPosition &pos = bdus.itemAt(bdus.size() - 1);
            size_t nextScan = pos.bduOffset + pos.bduSize;
            ...
            if (mFormat == NULL) {
                mFormat = MakeVC1CodecSpecificData(accessUnit);
            }
        ...
        BDUPosition pos;
        pos.bduOffset = bduStart - mBuffer->data();
        pos.bduSize = bduSize;
