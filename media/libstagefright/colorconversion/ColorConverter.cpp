...
// MStar Android Patch Begin
status_t ColorConverter::convertYUYV(
        const BitmapParams &src, const BitmapParams &dst) {
    ...
    const uint8_t *kAdjustedClip = initClip();

    uint16_t *dst_ptr = (uint16_t *)dst.mBits
        + dst.mCropTop * dst.mWidth + dst.mCropLeft;

    CHECK(((int) dst_ptr & 0x3) == 0); // must 4 bytes align

    const uint8_t *src_y =
        (const uint8_t *)src.mBits + ((src.mCropTop * src.mWidth + src.mCropLeft) << 1);
    ...
    for (size_t y = 0; y < src.cropHeight(); ++y) {
        for (size_t x = 0; x < src.cropWidth(); x += 2) {
            ...
            // clip range -278 .. 535
            size_t x2 = x << 1;
            signed y1 = (signed)src_y[x2] - 16;
            signed y2 = (signed)src_y[x2 + 2] - 16;

            signed u = (signed)src_y[x2 + 1] - 128;
            signed v = (signed)src_y[x2 + 3] - 128;
