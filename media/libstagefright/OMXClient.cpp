...
bool MuxOMX::IsSoftwareComponent(const char *name) {
    ...
    // do not create a local OMX object cause
    // 1. new OMX will construct both s/w and h/w omx plugins again in local process. however;
    // 2. we have only one set of h/w OMX components (on mediaplayerservice), which cannot be multiple instantiated.
    // FIXME: should consider creating another standalone s/w omx class to handle this requirement.
    return false;
