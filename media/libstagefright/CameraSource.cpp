...
// static const int64_t CAMERA_SOURCE_TIMEOUT_NS = 100000000LL;
...
static int32_t getColorFormat(const char* colorFormat) {
    ...
    // MStar Android Patch Begin
    if (!strcmp(colorFormat, CameraParameters::PIXEL_FORMAT_YUV420TILE)) {
        return OMX_COLOR_FormatYUV420SemiPlanar;
    }

    // Bypass Encoded ES from device
    if (!strcmp(colorFormat, CameraParameters::PIXEL_FORMAT_AVC)) {
       return OMX_COLOR_FormatEncodedAVC;
    }
    // MStar Android Patch End
...
CameraSource::CameraSource(
    ...
    : mCameraFlags(0),
      ...
      mCollectStats(false),
      // MStar Android Patch Begin
      camera_source_timeou_ns(3000000000LL) {
      // MStar Android Patch End
    mVideoSize.width  = -1;
    mVideoSize.height = -1;
...
status_t CameraSource::configureCamera(
    ...
    if (frameRate != -1) {
        ...
        if(mCameraId == 0 || mCameraId == 1) {
            ...
            CHECK(frameRate > 0 && frameRate <= 60);
        }
        // MStar Android Patch End
...
status_t CameraSource::initWithCameraAccess(
    ...
    // MStar Android Patch Begin
    mMeta->setInt32(kKeyCameraId,    cameraId);
    // MStar Android Patch End

    // MStar Android Patch Begin
    // sync mutex status to fix deadlock with dataCallbackTimestamp at CameraSource::reset()
    mCameraId = cameraId;
    if(mCameraId >= 3)
...
status_t CameraSource::startCameraRecording() {
    ...
    if (mCameraFlags & FLAGS_HOT_CAMERA) {
        mCamera->unlock();
        mCamera.clear();
        err = mCameraRecordingProxy->startRecording(new ProxyListener(this));
        if (err != OK)
            return err;
    } else {
...
status_t CameraSource::reset() {
    ALOGD("reset: E");
    // MStar Android Patch Begin
    // release mLock to fix deadlock with dataCallbackTimestamp
    ...
    while (!mFramesBeingEncoded.empty()) {
        if (NO_ERROR !=
            mFrameCompleteCondition.waitRelative(mLock,
                    mTimeBetweenFrameCaptureUs * 1000LL + camera_source_timeou_ns)) {
    ...
    // MStar Android Patch Begin
    // release mLock to fix deadlock with dataCallbackTimestamp
    // sync mutex status to fix deadlock with dataCallbackTimestamp
    mLock.unlock();
    if ((mCameraFlags & FLAGS_HOT_CAMERA) == 0) {
        Mutex* lock_client = Camera::getClientLockById(mCameraId);
        if (lock_client != NULL) {
            Mutex::Autolock alock(*lock_client);
        }
    }
    Mutex::Autolock autoLock(mLock);
    // MStar Android Patch End
