...
// MStar Android Patch Begin
      moldseqNo(0){
      mRtpDump = NULL;
      char dumpfile[PROPERTY_VALUE_MAX];
      if (property_get("wfd.dump.rtp", dumpfile, NULL)){
        if (strcmp(dumpfile, "on") == 0){
              int ret = 0;
              if(access("/data/rtpsink.ts", 0) == 0){
                ret = unlink("/data/rtpsink.ts");
              }
              if(ret == 0){
                mRtpDump = fopen("/data/rtpsink.ts", "ab+");
              } else {
                ALOGI("will not to creat dump file as :%s", strerror(errno));
              }

              if(mRtpDump == NULL)
                ALOGI("open or create rtpsink.ts failed: %s", strerror(errno));
        }
    }
...
status_t RTPSink::parseRTP(const sp<ABuffer> &buffer) {
    ...
    // MStar Android Patch Begin
    if(seqNo != moldseqNo+1){
        ALOGI("lost seqNo: %d   current seqNo: %d", moldseqNo + 1, seqNo);
    }
    ...
    if(mRtpDump != NULL){
        int count;
        count = fwrite(buffer->data(), 1, buffer->size(), mRtpDump);
        if (count <= 0) {
            ALOGI("write dump file error :%s", strerror(errno));
    }
    }
    // MStar Android Patch End
...
status_t RTPSink::connect(
        const char *host, int32_t remoteRtpPort) {
    ALOGI("connecting RTP sockets to %s:{%d}",
          host, remoteRtpPort);
