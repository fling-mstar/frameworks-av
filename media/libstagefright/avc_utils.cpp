...
void FindAVCDimensions(
    ...
    if (profile_idc == 100 || profile_idc == 110
        ...
        // temp remove for play hangup bug
        //CHECK_EQ(br.getBits(1), 0u);  // seq_scaling_matrix_present_flag
        if (br.getBits(1) != 0u) return;
...
// MStar Android Patch Begin
static void FindVC1Dimensions(
        const sp<ABuffer> &seqLayer,
        int32_t *width, int32_t *height,
        int32_t *sarWidth, int32_t *sarHeight) {
    ABitReader br(seqLayer->data() + 1, seqLayer->size() - 1);

    unsigned profile = br.getBits(2);
    unsigned level = br.getBits(3);
    unsigned chroma_format = br.getBits(2);
    unsigned frmrtq_postproc = br.getBits(3);
    unsigned bitrtq_postproc = br.getBits(5);
    unsigned postprocflag = br.getBits(1);
    unsigned max_coded_width = br.getBits(12);
    unsigned max_coded_height = br.getBits(12);
    *width = 2 * (max_coded_width + 1);
    *height = 2 * (max_coded_height + 1);

    unsigned pulldown = br.getBits(1);
    unsigned interlace = br.getBits(1);
    unsigned tfcntrflag = br.getBits(1);
    unsigned finterpflag = br.getBits(1);
    br.skipBits(2);
    unsigned display_ext = br.getBits(1);
    ...
    if (display_ext == 1) {
        ...
        if (br.getBits(1)) {  // aspect_ratio_flag
            unsigned sar_width = 0, sar_height = 0;

            unsigned aspect_ratio = br.getBits(4);
            if (aspect_ratio == 15 /* extendedSAR */) {
                sar_width = br.getBits(8);
                sar_height = br.getBits(8);
            } else if (aspect_ratio > 0 && aspect_ratio < 14) {
                static const int32_t kFixedSARWidth[] = {
                    1, 12, 10, 16, 40, 24, 20, 32, 80, 18, 15, 64, 160
                };
                ...
                sar_width = kFixedSARWidth[aspect_ratio - 1];
                sar_height = kFixedSARHeight[aspect_ratio - 1];
            }
...
sp<MetaData> MakeVC1CodecSpecificData(const sp<ABuffer> &accessUnit) {
    const uint8_t *data = accessUnit->data();
    size_t size = accessUnit->size();

    sp<ABuffer> seqLayer = FindNAL(data, size, 0x0F, NULL);
    ALOGV("seqLayer->size: %d", seqLayer->size());
    if (seqLayer == NULL) {
        return NULL;
    }
    ...
    FindVC1Dimensions(
            seqLayer, &width, &height, &sarWidth, &sarHeight);

    size_t stopOffset;
    sp<ABuffer> entryPointLayer = FindNAL(data, size, 0x0E, &stopOffset);
    CHECK(entryPointLayer != NULL);
#ifdef KeyAVCC // If codec need kKeyAVCC, fill csd buffer as following.
    // reference MakeAVCCodecSpecificData order.
    size_t csdSize =
        1 + 3 + 1 + 1
        + 2 * 1 + seqLayer->size() +
        + 1 + 2 * 1 +entryPointLayer->size();

    sp<ABuffer> csd = new ABuffer(csdSize);
    uint8_t *out = csd->data();

    *out++ = 0x01;  // configurationVersion
    memcpy(out, seqLayer->data() + 1, 1);  // profile/level...
    uint8_t profile = (out[0] & 0xC0) >> 6;
    uint8_t level = (out[0] & 0x38) >> 3;
    *out++ = profile;
    *out++ = 0;
    *out++ = level;
    ...
    *out++ = seqLayer->size() >> 8;
    *out++ = seqLayer->size() & 0xff;
    memcpy(out, seqLayer->data(), seqLayer->size());
    out += seqLayer->size();

    *out++ = 1;

    *out++ = entryPointLayer->size() >> 8;
    *out++ = entryPointLayer & 0xff;
    memcpy(out, entryPointLayer->data(), entryPointLayer->size());
#else
    // Fill seqLayer+entryPointLayer to csd buffer.
    size_t csdSize =
        seqLayer->size() +
        +entryPointLayer->size();
...
#ifdef DUMP_VC1_PARAM
    ALOGI("VC1 seq param set");
    hexdump(seqLayer->data(), seqLayer->size());
#endif

    sp<MetaData> meta = new MetaData;
    meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_WVC1);

    meta->setData(kKeyMsDecConfigData, 0, csd->data(), csd->size());
    meta->setInt32(kKeyWidth, width);
    meta->setInt32(kKeyHeight, height);
    ...
    if (sarWidth > 1 || sarHeight > 1) {
        ...
        ALOGI("found VC1 codec config (%d x %d, %d-profile level %d.%d) "
              "SAR %d : %d",
             width,
             height,
             profile,
