...
void SoftMP3::onQueueFilled(OMX_U32 portIndex) {
    ...
    while (!inQueue.empty() && !outQueue.empty()) {
        ...
        if ((decoderErr = pvmp3_framedecoder(mConfig, mDecoderBuf))
            ...
            // MStar Android Patch Begin
            if (inHeader->nFilledLen == 0 && mAnchorTimeUs == 0) {
               mConfig->outputFrameSize = 0;
               dummyInput = true;
               ALOGV("dummy input, pts and Output frame size is 0");
            }
            // MStar Android Patch End
            ...
            // MStar Android Patch Begin
            // Cannot output before sample rate be set, when errors happen in first frames
            mConfig->outputFrameSize = 0;
            // MStar Android Patch End
        } else if (mConfig->samplingRate != mSamplingRate
                || mConfig->num_channels != mNumChannels) {
            // MStar Android Patch Begin
            if (mIsSampleRateSet == false) {
        ...
        // MStar Android Patch Begin
        if (mIsFirst && (decoderErr == NO_DECODING_ERROR)) {
            mIsSampleRateSet = true;
        // MStar Android Patch End
            mIsFirst = false;
        ...
        // MStar Android Patch Begin
        if (dummyInput || outHeader->nFilledLen != 0) {
            outInfo->mOwnedByUs = false;
            outQueue.erase(outQueue.begin());
            outInfo = NULL;
            notifyFillBufferDone(outHeader);
