...
// Encode YUV420 tile data from Camera HAL
#ifdef BUILD_MSTARTV
#include <MsTypes.h>
#include <api_mfe_frmformat.h>
extern "C" {
extern void MsVdec_DetileYUV_Neon(void* pDstBuf, void* pSrcYBuf, void* pSrcUVBuf,
                           MS_U32 width, MS_U32 pitch, MS_U32 height, MS_BOOL bMVD);
}
#endif
...
// MStar Android Patch Begin
// Encode YUV420 tile data from Camera HAL
// convert tile to planar
#ifdef BUILD_MSTARTV
static void ConvertYUV420PackedPlanarToYUV420Planar(
        uint8_t *metainfo, uint8_t* outyuv,
        int32_t width, int32_t height) {
        MS_MfeFrameInfo_t* Mfeinfo = (MS_MfeFrameInfo_t* )metainfo;

        MsVdec_DetileYUV_Neon((void*)outyuv, (void* )Mfeinfo->inbuf.Cur_VirY0, (void*)Mfeinfo->inbuf.Cur_VirC0, width, width, height, false);
}
#endif
// MStar Android Patch End
...
OMX_ERRORTYPE SoftMPEG4Encoder::initEncParams() {
    ...
    if (mVideoColorFormat == OMX_COLOR_FormatYUV420SemiPlanar) {
        ...
        // MStar Android Patch Begin
        // mInputFrameData maybe set in kStoreMetaDataExtensionIndex handling
        if (mInputFrameData == NULL) {
            mInputFrameData = (uint8_t *) malloc((mVideoWidth * mVideoHeight * 3 ) >> 1);
        }
        // MStar Android Patch End
    ...
    while (!mSawInputEOS && !inQueue.empty() && !outQueue.empty()) {
        ...
        if (inHeader->nFilledLen > 0) {
            ...
            if (mStoreMetaDataInBuffers) {
                // MStar Android Patch Begin
                // Encode YUV420 tile data from Camera HAL
                // extract mstar video info and first 5 chars must be 0x00 0x00 0x00 0x01 0x47
#ifdef BUILD_MSTARTV
                char *metainfo = (char *) inInfo->mHeader->pBuffer;
                if ((metainfo[0] == 0x00)&&(metainfo[1] == 0x00) && (metainfo[2] == 0x00)
                    &&(metainfo[3] == 0x01)&&(metainfo[4] == 0x47)) {
                    mVideoColorFormat = OMX_COLOR_FormatYUV420PackedPlanar;
                } else if (inHeader->nFilledLen != 8) {
#else
            ...
            // Encode YUV420 tile data from Camera HAL
#ifdef BUILD_MSTARTV
            if (mVideoColorFormat == OMX_COLOR_FormatYUV420PackedPlanar) {
                ConvertYUV420PackedPlanarToYUV420Planar(
                    inputData, mInputFrameData, mVideoWidth, mVideoHeight);
                inputData = mInputFrameData;
            } else if (mVideoColorFormat != OMX_COLOR_FormatYUV420Planar) {
#else
...
uint8_t *SoftMPEG4Encoder::extractGrallocData(void *data, buffer_handle_t *buffer) {
    ...
    // MStar Android Patch Begin
    // Encode YUV420 tile data from Camera HAL
    // no need to extract Gralloc mem for MStar structure
#ifdef BUILD_MSTARTV
    if (OMX_COLOR_FormatYUV420PackedPlanar == mVideoColorFormat) {
        return (uint8_t *)data;
    }
#endif
    // MStar Android Patch End
