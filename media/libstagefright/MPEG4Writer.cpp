...
status_t MPEG4Writer::Track::start(MetaData *params) {
    ...
    // MStar Android Patch Begin
    // update encoding settings runtime 
    mMeta->findInt32(kKeyBitRate, &mBitrate);
    ALOGI("init bitrate is %d", mBitrate);
    // MStar Android Patch End
    pthread_create(&mThread, &attr, ThreadWrapper, this);
    pthread_attr_destroy(&attr);
...
status_t MPEG4Writer::Track::threadEntry() {
    ...
    // MStar Android Patch Begin
    // update encoding settings runtime 
    while (!mDone && (err = mSource->read(&buffer, &options)) == OK) {
        ...
        if(!mIsAudio) {
            ...
            if (mBitrate != bitrate) {
                ALOGI("new bitrate is %d", bitrate);
                options.setUpdateParam();
                mBitrate = bitrate;
            } else if(options.getUpdateParam()){
                ALOGI("clear refresh parameter");
                options.clearUpdateParam();
            }
        }
        // MStar Android Patch End
...
void MPEG4Writer::Track::writeVideoFourCCBox() {
    ...
    {
        bool success = mMeta->findCString(kKeySubMIMEType, &mime);
        CHECK(success);
        ALOGI("bypass mime type '%s'.", mime);
    } 
     // MStar Android Patch End   
