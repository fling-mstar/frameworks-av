...
public:
    ...
    // get information about client lock
    virtual Mutex* getClientLockById(int cameraId)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ICameraService::getInterfaceDescriptor());
        data.writeInt32(cameraId);
        remote()->transact(BnCameraService::GET_CAMERA_LOCK, data, &reply);
        return (Mutex*)reply.readInt32();
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        // MStar Android Patch Begin
        // sync mutex status to fix deadlock with dataCallbackTimestamp at CameraSource::reset()
        case GET_CAMERA_LOCK: {
            CHECK_INTERFACE(ICameraService, data, reply);
            reply->writeInt32((int32_t)getClientLockById(data.readInt32()));
            return NO_ERROR;
        } break;
        // MStar Android Patch End
