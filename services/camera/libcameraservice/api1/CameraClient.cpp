...
void CameraClient::setPreviewCallbackFlag(int callback_flag) {
    ...
    if (mPreviewCallbackFlag & CAMERA_FRAME_CALLBACK_FLAG_3DTEXTURE) {
    	enableMsgType(CAMERA_MSG_3DTEXTURE); 
	    return;
    } else {
    	disableMsgType(CAMERA_MSG_3DTEXTURE);
    }
...
status_t CameraClient::startCameraMode(camera_mode mode) {
    ...
    switch(mode) {
        ...
        case CAMERA_RECORDING_MODE:
            // MStar Android Patch Begin
            if ((mSurface == 0) && (mPreviewWindow == 0) && (0 == mCameraId)) {
            // MStar Android Patch End
