...
void *CameraService::waitForNextEvent(void *data) {
    ...
    while (1) {
        ...
        ALOGV("CameraService::Camera received uevent message: %s", buffer);
...
void CameraService::onFirstRef()
{
    ...
    // MStar Android Patch Begin
    if (CAMERA_ID_VE > mNumberOfCameras) {
        for (int i = mNumberOfCameras; i < CAMERA_ID_VE; ++i) {
            mStatusList[i] = ICameraServiceListener::STATUS_NOT_PRESENT;
        }
    }
    // MStar Android Patch End
}
...
int CameraService::getDeviceVersion(int cameraId, int* facing) {
    struct camera_info info;
    // MStar Android Patch Begin
    if (cameraId >= CAMERA_ID_VE && cameraId < MAX_CAMERAS) {
        *facing = 0;
...
status_t CameraService::validateConnect(int cameraId,
    ...
    // MStar Android Patch Begin
    if ((cameraId < 0) || (cameraId >= MAX_CAMERAS) || (cameraId == CAMERA_ID_GAP)) {
    // MStar Android Patch End
...
status_t CameraService::addListener(
    ...
    /* Immediately signal current status to this listener only */
    {
        ...
        for (int i = 0; i < MAX_CAMERAS; ++i) {
            if(mStatusList[i] != ICameraServiceListener::STATUS_NOT_PRESENT)
...
CameraService::BasicClient* CameraService::getClientByIdUnsafe(int cameraId) {
    // MStar Android Patch Begin
    if (cameraId < 0 || cameraId >= MAX_CAMERAS || cameraId == CAMERA_ID_GAP) return NULL;
    // MStar Android Patch End
