...
bool AudioFlinger::PlaybackThread::threadLoop()
{
    ...
    while (!exitPending())
    {
        ...
        { // scope for mLock
            ...
            if ((!(outDevice()&AUDIO_DEVICE_OUT_ALL_A2DP) && !mActiveTracks.size() && systemTime() > standbyTime) ||
                                   isSuspended()) {
...
void AudioFlinger::MixerThread::threadLoop_sleepTime()
{
    ...
    if (sleepTime == 0) {
        ...
        ALOGV_IF(mBytesWritten == 0 && (mMixerStatus == MIXER_TRACKS_ENABLED),
                "anticipated start");
    // MStar Android Patch Begin
    // Fixed A2DP has no sound when A2DP device connect TV
    // Trigger a2dp thread outwrite firstly when a2dp connected
    } else if (mMixerStatus == MIXER_IDLE) {
        // Let a2dp mixerthread not to threadLoop_sleep when a2dp connected
        if (outDevice() & AUDIO_DEVICE_OUT_ALL_A2DP) {
            ALOGI("MIXER_IDLE out_write to support A2DP output:outdevice is %d", outDevice());
            sleepTime = 0;
        }
    }
    // MStar Android Patch End
...
#if ENABLE_DUALMONO
    //get the DualMono Type
    String8 sDualMono = String8("DUALMONO");
    String8 sRetVal = String8("");
    char *chRet;
    bool bDualEnable = false;

    chRet = mOutput->stream->common.get_parameters(&mOutput->stream->common, sDualMono.string());
    sRetVal = String8(chRet);
    free(chRet);

    //FIX ME:the value should be fixed
    if (sRetVal.isEmpty() || (sRetVal == "false")) {
        bDualEnable = false;
    } else if (sRetVal == "true") {
        bDualEnable = true;
    }
#endif
// MStar Android Patch End
...
// Two mm play support speaker and line out
#if ENABLE_DUALMONO
            uint32_t u32MonoType = AudioMixer::DUALMONO_NONE;
            if (bDualEnable == true) {
                if (track->streamType() == AUDIO_STREAM_MUSIC_SUB) {
                    u32MonoType = AudioMixer::DUALMONO_RIGHT;
                } else {
                    u32MonoType = AudioMixer::DUALMONO_LEFT;
                }
            }
            mAudioMixer->setParameter(
                name,
                AudioMixer::TRACK,
                AudioMixer::DUALMONO_MODE, (void *)u32MonoType);
#endif
// MStar Android Patch End
...
track_is_ready: ;
    ...
    // MStar Android Patch Begin
    // mix buffer must be cleared if at least one track is connected to an
    // effect chain as in this case the mixer will not write to
    // mix buffer and track effects will accumulate into it
    if ((mBytesRemaining == 0) && ((mixedTracks != 0 && tracksWithEffect > 0) ||
