...
bool AudioFlinger::PlaybackThread::Track::isReady() const {
    ...
    /* For the extreme case that the shared the ClientProxy(AudioTrack) will be hung when the
    * ClientProxy tries to write data to shared buffer when the track is in RESUMING STATE
    * The Client Proxy hangs because the data buffer is full, but the Track is not ready, so data
    * cannot be consumed by Track for ServerProxy, so force to unlock the futex here
    */
    if((framesReady() < mFrameCount) && (mState == TrackBase::RESUMING)) {
        int32_t old = android_atomic_or(CBLK_FUTEX_WAKE, &mCblk->mFutex);
        if (!(old & CBLK_FUTEX_WAKE)) {
            ALOGV("Force to wake up the Futex Held by ClientProxy obtainBuffer!!!");
            (void)__futex_syscall3(&mCblk->mFutex, FUTEX_WAKE, 1);
        }
    }
