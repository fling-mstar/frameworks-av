...
// MStar Android Patch Begin
// Volume up/down, mute function
#if defined BUILD_MSTARTV_SN || defined BUILD_MSTARTV_MI
...
// Volume up/down, mute function
#if defined BUILD_MSTARTV_SN || defined BUILD_MSTARTV_MI
static sp<AudioManager> audiomanager = NULL;
static sp<PipManager> pipManager = NULL;

class AudioManagerClientListener : public AudioManagerListener
{
public:
...
void AudioManagerClientListener::PostEvent_Template(int32_t nEvt, int32_t ext1, int32_t ext2)
{
    ALOGV("PostEvent_Template()");
}

void AudioManagerClientListener::PostEvent_ApSetVolume(int32_t nEvt, int32_t ext1, int32_t ext2)
{
    ALOGV("PostEvent_ApSetVolume()");
}

void AudioManagerClientListener::PostEvent_SnServiceDeadth(int32_t nEvt, int32_t ext1, int32_t ext2)
{
    audiomanager = NULL;
}
#endif
...
static const char * const audio_interfaces[] = {
    ...
    // MStar Android Patch Begin
    // bluetooth mic from broadcom
    AUDIO_HARDWARE_MODULE_ID_BTMIC,
    // MStar Android Patch End
};
...
// Volume up/down, mute function
    ...
    if (audiomanager == NULL) {
        ...
        if (atoi(prop) == 1) {
            ...
            if (audiomanager == NULL) {
                ALOGE("Fail to connect to audio manager service.");
                return BAD_VALUE;
            }

            audiomanager->setListener(new AudioManagerClientListener());
        } else {
            return UNKNOWN_ERROR;
        }
    }

    audiomanager->setAudioVolume(0, (int8_t)((value * 1000)/10));
    if (pipManager == NULL) {
        pipManager = PipManager::connect();
        if (pipManager == NULL) {
           ALOGE("Fail to connect to pip manager service.");
           return BAD_VALUE;
        }
    }
    if (pipManager->getPipMode() == 0) {
        audiomanager->setAudioVolume(1, (int8_t)(value * 100.0)); // head phone
    }
#endif
// MStar Android Patch End
...
// Volume up/down, mute function
    ...
    if (muted == true) {
        audiomanager->enableMute(0x00004);
    } else {
        audiomanager->disableMute(0x00004);
    }
#endif
// MStar Android Patch End
...
// MStar Android Patch Begin
// bluetooth mic throug sco from broadcom
        } else if(param.get(String8(AudioParameter::keyBTMicRouting), value) == NO_ERROR) {
            for (size_t i = 0; i < mRecordThreads.size(); i++) {
                sp<RecordThread> thread = mRecordThreads.valueAt(i);
                audio_devices_t device = thread->inDevice();
                if(audio_is_bluetooth_mic_device(device)) {
...
// AudioRecord::set INPUT equivalent
audio_io_handle_t AudioFlinger::openInputRouter(audio_io_handle_t input,
        audio_io_handle_t output)
{

    audio_io_handle_t rt_io_handle = 0;

    ALOGD("%s:: calling Routing Thread Initializer (%d -> %d)",
        __FUNCTION__, input, output);

    mRoutingThread =
        AudioFlinger::RoutingThread::Initialize(this, input, output,
                                                &rt_io_handle);
                                          ...
                                          audio_channel_mask_t *pChannelMask)
{
    ...
    // MStar Android Patch Begin
    // For pass Android XTS testRecordFromRemoteSubmix test item.
    // We should use AUDIO_DEVICE_IN_BUILTIN_MIC to find device.
    if (*pDevices == AUDIO_DEVICE_IN_REMOTE_SUBMIX) {
        inHwDev = findSuitableHwDev_l(module, AUDIO_DEVICE_IN_BUILTIN_MIC);
    }
    else {
...
status_t AudioFlinger::closeInput_nonvirtual(audio_io_handle_t input)
{
    // MStar Android Patch Begin
    // porting from brcm
    audio_io_handle_t rt_input = (mRoutingThread) ? mRoutingThread->mInput : -1;

    // stop any open routing tracks
    if (input == rt_input) {
        if (mRoutingThread->mOutputThread != NULL) {
           ALOGV("%s: Remove Output track from PlaybackThread %d",
        ...
        ALOGV("%s: exit routingthread loop", __FUNCTION__);
...
AudioFlinger::RoutingThread *AudioFlinger::checkRoutingThread_l(audio_io_handle_t input) const
{
    int wait_ms = 5;
    int attempts = 3;
    ...
    // check handle (if not broadcast handle)
    if ((input != 0) && (input != mRoutingThread->mInput))
        return NULL;
