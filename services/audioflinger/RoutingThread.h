...
class RoutingThread : public Thread {
    ...
    RoutingThread (audio_io_handle_t input, audio_io_handle_t output);
    ...
    bool InitializeInternal(AudioFlinger *pAudioFlinger);
    static RoutingThread *Initialize(AudioFlinger *pAudioFlinger,
        ...
        // INPUT (AudioRecord equivalent)
        status_t read(void* buffer, size_t userSize);
        ...
        audio_track_cblk_t*           mInputCblk;
        sp<AudioRecordClientProxy>    mInputProxy;
        sp<IMemory>                   mInputCblkMemory;
        ...
        status_t obtainBufferOut(Buffer* audioBuffer, const struct timespec *requested,
        struct timespec *elapsed = NULL, size_t *nonContig = NULL);

        void     releaseBufferOut(Buffer* audioBuffer);
