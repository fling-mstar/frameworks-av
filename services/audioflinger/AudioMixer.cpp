...
// Two mm play support speaker and line out
#if ENABLE_DUALMONO
        t->u32DualMonoMode = DUALMONO_NONE;
#endif
// MStar Android Patch End
        t++;
    }
...
#if ENABLE_DUALMONO
        case DUALMONO_MODE:
            track.u32DualMonoMode = valueInt;
            if (valueInt != DUALMONO_NONE) {
                if (false == mState.bDualMonoEnable) {
                    mState.mLeftChannelHook = process__validate;
                    mState.u32LeftChannelTracks = 0 ;
                    mState.mRightChannelHook = process__validate;
                    mState.u32RightChannelTracks = 0;
                    mState.bDualMonoEnable = true;
                }
            } else {
                if (true == mState.bDualMonoEnable) {
                    mState.hook = process__validate;
                    mState.bDualMonoEnable = false;
                }
            }
            ALOGV("setParameter(TRACK, DUALMONO, %d)", valueInt);
            break;
#endif
// MStar Android Patch End
        default:
...
// Two mm play support speaker and line out
#if ENABLE_DUALMONO
    if (mState.bDualMonoEnable) {
        int32_t* pS32MixBuffer = NULL;
        int32_t* pS32BackUpBuffer = NULL;
        uint32_t u32AfterChange = 0;

        if ((mState.needsChanged != 0) || (mState.enabledTracks != (mState.u32LeftChannelTracks | mState.u32RightChannelTracks))) {
            //1.calculate the real enableTrack after needschanged
            uint32_t u32changed = mState.needsChanged;
            uint32_t u32enabledTracks = 0;
            uint32_t u32disabledTracks = 0;

            uint32_t  u32RightTrack = 0;
            uint32_t  u32LeftTrack = 0;
            u32AfterChange = mState.enabledTracks;

            while (u32changed) {
                const int i = 31 - __builtin_clz(u32changed);
                const uint32_t mask = 1<<i;
                u32changed &= ~mask;
                track_t& t = mState.tracks[i];
                (t.enabled ? u32enabledTracks : u32disabledTracks) |= mask;
            }
            u32AfterChange &= ~u32disabledTracks;
            u32AfterChange |=  u32enabledTracks;
            ALOGV("original enabledTrack:%x, after enabledTrack:%x, needschanged:%x",mState.enabledTracks,u32AfterChange,mState.needsChanged);

            //2.find the right channel stream and left channel stream
            uint32_t u32enableTracks = u32AfterChange;
            while (u32enableTracks > 0) {
                const int i = 31 - __builtin_clz(u32enableTracks);
                track_t& t = mState.tracks[i];
                uint32_t mask = ( 1 << i );

                u32enableTracks &= ~(1<<i);

                if (t.u32DualMonoMode == DUALMONO_RIGHT) {
                    u32RightTrack |= mask;
                } else {
                    u32LeftTrack |= mask;
                }
            }
            mState.u32LeftChannelTracks = u32LeftTrack;
            mState.mLeftChannelHook = process__validate;
            mState.u32RightChannelTracks = u32RightTrack;
            mState.mRightChannelHook = process__validate;
            mState.needsChanged = 0;
        } else {
            u32AfterChange = mState.u32LeftChannelTracks | mState.u32RightChannelTracks;
        }
        //find the buffer to store the mix result
        if (u32AfterChange > 0) {
            const int i = 31 - __builtin_clz(u32AfterChange);
            track_t& t = mState.tracks[i];
            pS32MixBuffer = t.mainBuffer;
        }
        ALOGV("enableTracks %x u32RightChannelTracks % u32LeftChannelTracks %x",mState.enabledTracks,mState.u32RightChannelTracks, mState.u32LeftChannelTracks);
        
        //3.the first time to call process to deal with left channel sound
        if ((mState.u32LeftChannelTracks) > 0) {
            mState.enabledTracks =  mState.u32LeftChannelTracks;
            mState.mLeftChannelHook(&mState,pts);
            mState.mLeftChannelHook = mState.hook;

            //chang sound to left channel sound
            for (uint32_t i = 0 ; i < mState.frameCount ; i ++) {
                pS32MixBuffer[i] = (((pS32MixBuffer[i] >> 16) + ((pS32MixBuffer[i] << 16) >> 16)) >> 1) & 0x0000FFFF;
            }
        }
    
        //4.deal with right channel sound
        if (mState.u32RightChannelTracks > 0) {
            //back up the left channel sound
            if (mState.u32LeftChannelTracks > 0) {
                pS32BackUpBuffer = new int32_t[mState.frameCount];
                memcpy(pS32BackUpBuffer,pS32MixBuffer,mState.frameCount*4);
                memset(pS32MixBuffer,0,mState.frameCount*4);
            }

            ALOGV("the second process enabledTracks %x",mState.u32RightChannelTracks);
            mState.enabledTracks = mState.u32RightChannelTracks ;
            mState.mRightChannelHook(&mState,pts);
            mState.mRightChannelHook = mState.hook;

            //copy stere sound to right channel sound and mix left channel and right channel
            if (mState.u32LeftChannelTracks > 0) {
                for (uint32_t i = 0 ; i < mState.frameCount ; i ++) {
                    pS32MixBuffer[i] = ((((pS32MixBuffer[i] >> 16) + ((pS32MixBuffer[i] << 16) >> 16)) >> 1) << 16) | pS32BackUpBuffer[i];
                }
            } else {
                for (uint32_t i = 0 ; i < mState.frameCount ; i ++) {
                    pS32MixBuffer[i] = (((pS32MixBuffer[i] >> 16) + ((pS32MixBuffer[i] << 16) >> 16)) >> 1) << 16;
                }
            }
            if (pS32BackUpBuffer != NULL) {
                delete pS32BackUpBuffer;
            }
        }
        //recover the data
        mState.enabledTracks = u32AfterChange;
        ALOGV("the recover enableTracks %x",mState.enabledTracks);
    } else {
